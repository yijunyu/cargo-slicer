...
| `CARGO_SLICER_PHASE` | unset | Phase: `analysis`, `build`, `lazy-metadata`, `lazy-codegen`, or empty (legacy single-pass) |
| `CARGO_SLICER_PARSER` | `syn` | Pre-analysis parser backend: `syn`, `fast`, or `ctags` |

#### Lazy Compilation (Stage 2)

| Variable | Default | Description |
|----------|---------|-------------|
| `CARGO_SLICER_LAZY` | unset | Set to `1` to enable metadata-first two-pass compilation |

#### MIR-Precise Analysis (Stage 3)

| Variable | Default | Description |
|----------|---------|-------------|
| `CARGO_SLICER_MIR_PRECISE` | unset | Set to `1` to enable MIR-precise whole-program analysis in binary crate |
| `CARGO_SLICER_WORKSPACE_CRATES` | unset | Comma-separated list of workspace crate names for MIR harvesting |
