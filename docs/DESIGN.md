...
## Upstream Integration: `-Z dead-fn-elimination`

The virtual slicing logic has been extracted into a self-contained rustc patch proposal at
[`src/upstream_patch/dead_fn_elim.rs`](../src/upstream_patch/dead_fn_elim.rs). This is a
**Level A** in-tree integration: move the three query overrides verbatim into
`compiler/rustc_mir_transform/` behind a new unstable `-Z dead-fn-elimination` flag, with
no new compiler infrastructure required.

### Motivation

The current `RUSTC_WRAPPER` approach has four persistent pain points that in-tree
integration eliminates entirely:

| Pain Point | Root Cause | In-Tree Solution |
|------------|-----------|-----------------|
| Nightly API breakage | `rustc_private` is unstable | Code is internal — no breakage |
| ABI mismatch per nightly | `librustc_driver.so` symbol hash | No `.so` boundary at all |
| Per-nightly driver bundles | Separate `cargo-slicer-rustc` binary | Single nightly toolchain flag |
| `OnceLock`/`Mutex<Option>` global state | Can't pass state through query provider `fn` pointers | `thread_local!` with full closure captures |

### What Changes in rustc (Three Files)

#### 1. `compiler/rustc_session/src/options.rs`

Add one line to the `Z_OPTIONS` macro:
...
#### 2. `compiler/rustc_mir_transform/src/dead_fn_elim.rs` (new file)

Copy [`src/upstream_patch/dead_fn_elim.rs`](../src/upstream_patch/dead_fn_elim.rs) verbatim,
removing the stub type aliases at the bottom (not needed in-tree). The module contains:

- **`thread_local!` state** — replaces `OnceLock<Mutex<Option<T>>>` globals. Each rustc
  process gets a fresh thread-local; no reset-between-compilations logic needed.
- **`register_providers(providers)`** — overrides two queries:
  - `optimized_mir`: intercepts each function's MIR, replaces unreachable bodies with a
    single `Unreachable` terminator. O(1) lookup via pre-built `HashSet<u64>` of DefId indices.
  - `collect_and_partition_mono_items`: removes stubbed items from all CodegenUnits so
    LLVM generates zero IR for them.
- **`run_analysis(tcx)`** — called from `after_analysis()`. Scans all local MIR bodies
  via `tcx.optimized_mir()` (in-memory, no disk caches), runs BFS from entry-point seeds,
  builds O(1) DefId caches.
- **`is_safe_to_stub(tcx, def_id)`** — the 11-point safety checklist (see below).
- **`build_stub_body(tcx, orig)`** — constructs minimal MIR: one BB, `Unreachable` terminator,
  original local declarations preserved for type-checking.

#### 3. `compiler/rustc_driver_impl/src/lib.rs`

Two small additions:

```rust
// Site 1: in run_compiler(), after session creation:
if sess.opts.unstable_opts.dead_fn_elimination {
    rustc_mir_transform::dead_fn_elim::register_providers(&mut local_providers);
}

// Site 2: after_analysis callback:
...
### Key Simplifications vs. Driver-Based Approach

**State management**: The driver uses `OnceLock<Mutex<HashMap<…>>>` and `Mutex<Option<HashSet<…>>>` globals
because query providers must be `fn` pointers (no captures). In-tree, the same constraint
applies to the provider fields — but the BFS result only needs to cross from `after_analysis`
to `optimized_mir`, which `thread_local!` handles cleanly:

```
after_analysis()         optimized_mir(tcx, def_id)
    │                           │
    ├─ run_bfs() → marked set   ├─ MARKED_DEF_IDS.with(|m| m.contains(&idx))
    └─ MARKED_DEF_IDS.set(…)   └─ if not marked + safe → return stub
```

**Cross-crate analysis**: The driver writes `.slicer-cache/*.cache` files to pass marked sets
between crate compilations. In-tree, `tcx.optimized_mir(extern_def_id)` returns any extern
crate's MIR directly (the binary crate's `TyCtxt` has all dependencies loaded). No disk
files needed.

**No source hashing**: The driver computes FNV-1a hashes of `.rs` files to validate caches.
In-tree, incremental compilation's query fingerprinting handles this automatically.

### The 11-Point Safety Checklist

`is_safe_to_stub` is the correctness core. All 11 checks must pass; any single failure
means "keep the original function body":

| # | Check | Rationale |
|---|-------|-----------|
| 1 | `def_id.is_local()` | Only stub items in the current crate |
| 2 | `DefKind::Fn` or `DefKind::AssocFn` | Only stub functions, not types/statics |
| 2a | Vtable check for `AssocFn` | `dyn Trait` dispatch can call any impl |
| 3 | `vis.is_public() && !is_binary_crate` | Pub lib items have unseen downstream callers |
| 4 | `NO_MANGLE`, `USED_LINKER`, `symbol_name`, `linkage` | Linker-visible symbols |
| 5 | `Drop::drop` impl (via lang item) | Compiler inserts drop calls outside BFS |
| 6 | Entry point (`fn main`) | Must execute |
| 7 | `#[test]`, `#[bench]` | Test harness calls outside BFS |
| 8 | `fn_sig().safety().is_unsafe()` | Unsafe fns callable via transmuted fn ptrs |
| 9 | `generics.count() > 0` | Pre-mono BFS can't see monomorphization sites |
| 10 | `asyncness.is_async()` | Async fn bodies are coroutine state machines |
| 11 | Sig contains `FnPtr` or `dyn Trait` | Dynamic dispatch via signature types |

### Usage (after rustc integration)

```bash
# No RUSTC_WRAPPER, no cargo-slicer-rustc, no per-nightly bundle:
RUSTFLAGS="-Z dead-fn-elimination" cargo +nightly build --release

# Verify: fewer symbols in output binary
nm target/release/mybin | grep -c ' T '
```

### Implementation Status

The patch is implemented and tested in cargo-slicer's source tree:

- [`src/upstream_patch/dead_fn_elim.rs`](../src/upstream_patch/dead_fn_elim.rs) — full implementation
- [`src/upstream_patch/mod.rs`](../src/upstream_patch/mod.rs) — integration notes
- 9 unit tests covering BFS correctness (cycles, transitive edges, disconnected subgraphs)
- The pure `run_bfs()` function compiles and tests without `rustc_private`
- The full rustc implementation is in `FULL_IMPLEMENTATION_CODE` (a string constant that
  compiles when placed inside rustc's source tree)

**Next step**: Submit as a rustc MCP (Major Change Proposal) or RFC. The patch is ~350 lines
of logic (the rest is documentation and the BFS algorithm that's already tested).
