...
# Locate cargo-slicer as a standalone binary (not via cargo subcommand dispatch)
# Search PATH plus the default install dir in case PATH is not yet updated
SLICER=""
...
for _dir in $(echo "$PATH:$_CARGO_BIN" | tr ':' ' '); do
    if [[ -f "$_dir/cargo-slicer" && -x "$_dir/cargo-slicer" ]]; then
        # Make sure it's not just a symlink to cargo
