...
# On Linux x86_64, prefer the musl static binary (no glibc dependency)
if [[ "$OS" == "Linux" && "$ARCH_LABEL" == "x86_64" ]]; then
    MUSL_TARGET="${ARCH_LABEL}-unknown-linux-musl"
    MUSL_ARCHIVE="cargo-slicer-${MUSL_TARGET}.tar.gz"
    TARGET="$MUSL_TARGET"
...
if [[ -f "$CANDIDATE" ]]; then
    ...
    if ! "$CANDIDATE" --help &>/dev/null; then
        echo "" >&2
        echo "Warning: downloaded binary is not compatible with this system (likely glibc mismatch)." >&2
        echo "Falling back to building from source..." >&2
        echo "" >&2
        cargo install --git "https://github.com/$REPO" --bin cargo-slicer
        echo ""
        echo "  Installed (from source): $INSTALL_DIR/cargo-slicer"
        # Still install scripts if present
        for f in cargo-slicer.sh cargo-slicer.ps1 install.sh; do
            if [[ -f "$TMPDIR/$f" || -f "$TMPDIR/cargo-slicer/$f" ]]; then
                src="${TMPDIR}/cargo-slicer/$f"; [[ -f "$src" ]] || src="$TMPDIR/$f"
                cp "$src" "$INSTALL_DIR/$f" && chmod +x "$INSTALL_DIR/$f"
            fi
        done
        SKIP_BIN_INSTALL=1
    fi
...
if [[ -f "$CALLER_DIR/Cargo.toml" ]]; then
    SLICER_SH=""
