...
echo ""
echo "Transparent mode — make plain \`cargo\` accelerate everywhere:"
echo ""
echo "    cargo-slicer enable     # writes ~/.cargo/config.toml; then just use \`cargo\`"
echo "    cargo-slicer disable    # revert"
echo "    cargo-slicer status     # check"
echo ""
echo "Rust project (4 steps, fully automatic):"
echo "  1. cargo-warmup serves pre-compiled .rlib/.so for serde/syn/tokio/..."
echo "     (registry deps — up to 2.3× on cached single-crate builds, 1.1–1.7× incremental)"
