...
fn find_rustc_driver_binary() -> Result<PathBuf, String> {
    ...
    let debug_path = PathBuf::from("target/debug").join(&driver_name);
    if debug_path.exists() {
        return Ok(debug_path);
    }

    // Check in target/release
    let release_path = PathBuf::from("target/release").join(&driver_name);
    ...
    if let Ok(output) = Command::new(which_cmd).arg(&driver_name).output() {
        if output.status.success() {
