...
*   Adapted from universal-ctags for precc's ctags API.
*/
...
/* RustKinds array moved to Rust - see RUST_KINDS in parse.rs
 * Access via rs_getRustKinds() or RustKinds macro
 */
...
void parseBlock (lexerState *lexer, boolean delim, int kind, vString *scope);
...
void writeCurTokenToStr (lexerState *lexer, vString *out_str)
{
	switch (lexer->cur_token)
	{
		...
		case TOKEN_WHITESPACE:
			rs_vStringPut(out_str, ' ');
			break;
		case TOKEN_LSHIFT:
			rs_vStringCatS(out_str, "<<");
			break;
		case TOKEN_RSHIFT:
			rs_vStringCatS(out_str, ">>");
			break;
		case TOKEN_RARROW:
			...
			break;
		default:
			rs_vStringPut(out_str, lexer->cur_token);
	}
}

/* Reads a character from the file */
void advanceChar (lexerState *lexer)
{
...
void advanceNChar (lexerState *lexer, int n)
{
	while (n--)
		advanceChar(lexer);
}
...
void advanceAndStoreChar (lexerState *lexer)
{
	if (vStringLength(lexer->token_str) < MAX_STRING_LENGTH)
		rs_vStringPut(lexer->token_str, lexer->cur_c);
	advanceChar(lexer);
}

/* Rust FFI helper functions */
extern int rs_isWhitespace(int c);
extern int rs_isIdentStart(int c);
extern int rs_isIdent(int c);

void scanWhitespace (lexerState *lexer)
{
	while (rs_isWhitespace(lexer->cur_c))
		advanceChar(lexer);
}
...
void scanComments (lexerState *lexer)
{
	/* // */
	if (lexer->next_c == '/')
	{
...
void scanIdentifier (lexerState *lexer)
{
	rs_vStringClear(lexer->token_str);
	do
	{
		advanceAndStoreChar(lexer);
	} while(lexer->cur_c != EOF && rs_isIdent(lexer->cur_c));
}
...
void scanString (lexerState *lexer)
{
	rs_vStringClear(lexer->token_str);
	advanceAndStoreChar(lexer);
	while (lexer->cur_c != EOF && lexer->cur_c != '"')
 ...
 * hashes must match */
void scanRawString (lexerState *lexer)
{
 ...
 * lifetimes. */
void scanCharacterOrLifetime (lexerState *lexer)
{
	rs_vStringClear(lexer->token_str);
	advanceAndStoreChar(lexer);
...
int rustAdvanceToken (lexerState *lexer, boolean skip_whitspace)
{
	...
	while (lexer->cur_c != EOF)
	{
		if (rs_isWhitespace(lexer->cur_c))
		{
	...
	while (lexer->cur_c != EOF)
	{
		...
		else if (rs_isIdentStart(lexer->cur_c))
		{
...
{
	advanceNChar(lexer, 2);
	lexer->token_str = rs_vStringNew();

	if (lexer->cur_c == '#' && lexer->next_c == '!')
		scanComments(lexer);
	rustAdvanceToken(lexer, TRUE);
}

void deInitLexer (lexerState *lexer)
{
	rs_vStringDelete(lexer->token_str);
	lexer->token_str = NULL;
}

void addTag (vString* ident, const char* arg_list, int kind, unsigned long line, fpos_t pos, vString *scope, int parent_kind)
{
	...
	if (parent_kind != K_NONE && vStringLength(scope) > 0)
	{
...
void skipUntil (lexerState *lexer, int goal_tokens[], int num_goal_tokens)
{
	int angle_level = 0;
	int paren_level = 0;
	int brace_level = 0;
...
void parseFn (lexerState *lexer, vString *scope, int parent_kind)
{
	...
	name = rs_vStringNewCopy(lexer->token_str);
	arg_list = rs_vStringNew();
	...
	if (valid_signature)
	{
		rs_vStringStripTrailing(arg_list);
	...
	rs_vStringDelete(arg_list);
}

/* Mod format:
 * "mod" <ident> "{" [<body>] "}"
 * "mod" <ident> ";"*/
void parseMod (lexerState *lexer, vString *scope, int parent_kind)
{
	rustAdvanceToken(lexer, TRUE);
	if (lexer->cur_token != TOKEN_IDENT)
		return;
 ...
 */
void parseTrait (lexerState *lexer, vString *scope, int parent_kind)
{
	int goal_tokens[] = {'{'};
...
void skipTypeBlock (lexerState *lexer)
{
	if (lexer->cur_token == '<')
	{
...
void parseQualifiedType (lexerState *lexer, vString* name)
{
	while (lexer->cur_token != TOKEN_EOF)
	{
		if (lexer->cur_token == TOKEN_IDENT)
 ...
 */
void parseImpl (lexerState *lexer, vString *scope, int parent_kind)
{
	unsigned long line;
	fpos_t pos;
	vString *name;
 ...
 * "static" ["mut"] <ident>
 */
void parseStatic (lexerState *lexer, vString *scope, int parent_kind)
{
	rustAdvanceToken(lexer, TRUE);
	if (lexer->cur_token != TOKEN_IDENT)
		return;
 ...
 * "const" <ident>
 */
void parseConst (lexerState *lexer, vString *scope, int parent_kind)
{
	rustAdvanceToken(lexer, TRUE);
	if (lexer->cur_token != TOKEN_IDENT)
		return;
 ...
 * "type" <ident>
 */
void parseType (lexerState *lexer, vString *scope, int parent_kind)
{
	rustAdvanceToken(lexer, TRUE);
	if (lexer->cur_token != TOKEN_IDENT)
		return;
 ...
 * */
void parseStructOrEnum (lexerState *lexer, vString *scope, int parent_kind, boolean is_struct)
{
	...
	if (lexer->cur_token == '{')
	{
		vString *field_name = rs_vStringNew();
		while (lexer->cur_token != TOKEN_EOF)
		{
			int goal_tokens2[] = {'}', ','};
			...
			if (lexer->cur_token == TOKEN_IDENT)
			{
				    ...
				    || strcmp(vStringValue(lexer->token_str), "pub") == 0)
				{
					...
					/* Skip the visibility specifications.
					 * https://doc.rust-lang.org/reference/visibility-and-privacy.html */
					if (lexer->cur_token == '(')
					{
						int vis_goal[] = {')'};
						rustAdvanceToken(lexer, TRUE);
						skipUntil(lexer, vis_goal, 1);
						rustAdvanceToken(lexer, TRUE);
					}
...
void rustSkipMacro (lexerState *lexer)
{
	int level = 0;
	int plus_token = 0;
	int minus_token = 0;
 ...
 */
void parseMacroRules (lexerState *lexer, vString *scope, int parent_kind)
{
	rustAdvanceToken(lexer, TRUE);
...
void parseBlock (lexerState *lexer, boolean delim, int kind, vString *scope)
{
	...
	while (lexer->cur_token != TOKEN_EOF)
	{
		if (lexer->cur_token == TOKEN_IDENT)
		{
			...
			rs_vStringTruncate(scope, old_scope_len);
		}
		else if (lexer->cur_token == '{')
		{
			level++;
...
extern parserDefinition *RustParser (void)
{
	...
	def->kinds = rs_getRustKinds();
	def->kindCount = rs_getRustKindsCount();
