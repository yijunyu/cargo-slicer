...
/*
*   GLOBAL VARIABLES
*   TagFile is now owned by Rust - access via rs_getTagFile() macro
*/
extern tagFile *rs_getTagFile(void);
#define TagFile (*rs_getTagFile())

/*
*   FUNCTION PROTOTYPES
*/
/* freeTagFileResources now implemented directly in Rust as rs_freeTagFileResourcesDirect */
...
*   These are implemented in src/ctags_rs/entry.rs
*/
extern tagEntryInfo *rs_tagEntryInfoNew (void);
extern void rs_tagEntryInfoDelete (tagEntryInfo *entry);
extern const char *rs_tagEntryInfoGetName (const tagEntryInfo *entry);
extern char rs_tagEntryInfoGetKind (const tagEntryInfo *entry);
extern unsigned long rs_tagEntryInfoGetLineNumber (const tagEntryInfo *entry);
