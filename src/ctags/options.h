...
/*
*   GLOBAL VARIABLES
*   Option is owned by Rust, accessed via rs_getOption() macro
*/
extern optionValues *rs_getOption (void);
#define Option (*rs_getOption())

/* Excluded and OptionFiles are now Rust-owned in options.rs */
extern stringList *rs_getExcluded (void);
extern void rs_setExcluded (stringList *excluded);
extern stringList **rs_getExcludedPtr (void);
extern stringList *rs_getOptionFiles (void);
extern void rs_setOptionFiles (stringList *option_files);
extern stringList **rs_getOptionFilesPtr (void);
#define Excluded (rs_getExcluded())
#define OptionFiles (rs_getOptionFiles())
...
/* freeOptionResources now implemented directly in Rust as rs_freeOptionResourcesDirect */
extern void rs_freeOptionResourcesDirect (void);

/*
*   RUST FFI FUNCTION DECLARATIONS
*   These are implemented in src/ctags_rs/options.rs
*/
extern optionValues *rs_optionValuesNew (void);
extern void rs_optionValuesDelete (optionValues *options);
extern int rs_optionIsAppend (const optionValues *options);
extern int rs_optionIsEtags (const optionValues *options);
extern int rs_optionIsVerbose (const optionValues *options);
extern sortType rs_optionGetSortType (const optionValues *options);
extern exCmd rs_optionGetLocate (const optionValues *options);
extern cookedArgs *rs_cookedArgsNew (void);
extern void rs_cookedArgsDelete (cookedArgs *args);
extern void rs_cArgDelete (cookedArgs *current);
extern int rs_cookedArgsIsOption (const cookedArgs *args);
extern int rs_cookedArgsIsLongOption (const cookedArgs *args);
