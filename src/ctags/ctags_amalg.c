...
#ifdef _WIN32
/* Avoid #include <windows.h> which conflicts with ctags macros (File, Signature, boolean).
 * Use a simple volatile flag with InterlockedCompareExchange for one-time init instead. */
static volatile long hashTableMutexInit = 0;
static volatile long hashTableMutexLock = 0;

static void hashTableLock(void) {
	while (_InterlockedCompareExchange(&hashTableMutexLock, 1, 0) != 0) {
		/* spin */
	}
}
static void hashTableUnlock(void) {
	_InterlockedExchange(&hashTableMutexLock, 0);
}
#else
...
/* _findfirst returns intptr_t on modern MSVC */
#ifdef HAVE__FINDFIRST
typedef intptr_t findfirst_t;
#endif
