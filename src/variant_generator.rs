...
impl VariantGenerator {
    ...
    /// Rewrite [dependencies] section with path deps
    ...
    /// Helper to format dependency line, handling path deps vs. version specs
    fn format_dependency_line(&self, crate_name: &str, dep_value: &str) -> String {
        if self.should_use_path_dep(crate_name) {
            ...
            let path = format!("{}/{}-sliced", abs_sliced_dir.display(), crate_name);
        ...
        } else if self.fallback_crates.contains(crate_name) {
