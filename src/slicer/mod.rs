...
// Allow warnings to not block compilation with different feature sets
#![warn(warnings)]
...
pub mod root_set;
pub mod project_slicer;
pub mod text_extractor;
pub mod ctags_parser;
pub mod text_dependencies;
pub mod text_deleter;
...
pub fn slice_crate(
    ...
    let t2 = Instant::now();
    let (graph, index, public_reexports) = dependency_graph::build_graph(
        output_dir,
        crate_name,
        config,
        Some(used_items),  // Pass entry points for incremental graph building
    )?;
    ...
    let t4 = Instant::now();
    let stats = if config.features.use_ctags {
        // Use text-based deleter (no syn parsing)
        if config.verbose {
            println!("  📝 Using text-based deletion (ctags mode)...");
        }
        text_deleter::delete_unused_text_based(&index, &used, output_dir, config)?
    } else {
        // Use AST-based deleter (default)
        deleter::delete_unused(&index, &used, output_dir, config)?
    };
