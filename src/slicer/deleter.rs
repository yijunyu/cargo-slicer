...
use rayon::prelude::*;
// use regex::Regex; // No longer needed - using semantic occurrence detection
...
/// Count occurrences of a name in content
/// 
/// SEMANTIC FIX: We use a conservative approach that catches more usage patterns:
/// - Direct calls: `foo()`
/// - Comparisons: `val < CONST`  
/// - Pattern matching: `Some(CONST)`
/// - Function pointers: `let f = foo;`
/// - Method chains: `foo.bar()` (where bar is the name)
///
/// We accept some false positives (e.g., "foo" in "foobar") to avoid false negatives
/// (deleting actually-used items). The trade-off favors correctness over LOC reduction.
fn count_occurrences(name: &str, content: &str) -> usize {
    // Pattern 1: Name followed by ( - function call
    let call_pattern = format!("{}(", name);
    // Pattern 2: Name surrounded by non-identifier chars - standalone usage
    // We check for: space, tab, newline, punctuation, operators
    let mut standalone_count = 0;
    let name_len = name.len();
    
    for (idx, window) in content.as_bytes().windows(name_len).enumerate() {
        if window == name.as_bytes() {
            // Check char before (if exists)
            let before_ok = idx == 0 || {
                let before = content.as_bytes()[idx - 1];
                // Non-identifier char: space, tab, newline, punctuation, operators, brackets
                before.is_ascii_whitespace() || 
                matches!(before, b'(' | b'[' | b'{' | b',' | b';' | b':' | b'=' | b'!' | 
                         b'<' | b'>' | b'+' | b'-' | b'*' | b'/' | b'&' | b'|' | b'^' | 
                         b'~' | b'%' | b'?' | b'.' | b'#')
            };
            
            // Check char after (if exists)
            let after_idx = idx + name_len;
            let after_ok = after_idx >= content.len() || {
                let after = content.as_bytes()[after_idx];
                // Non-identifier char: space, tab, newline, punctuation, operators, brackets
                after.is_ascii_whitespace() || 
                matches!(after, b')' | b']' | b'}' | b',' | b';' | b':' | b'=' | b'!' |
                         b'<' | b'>' | b'+' | b'-' | b'*' | b'/' | b'&' | b'|' | b'^' |
                         b'~' | b'%' | b'?' | b'.' | b'(' | b'[')
            };
    ...
    let call_count = content.matches(&call_pattern).count();
    call_count + standalone_count
}
                ...
                match item {
                    ...
                    Item::Const(c) => {
                        if features.delete_private_const {
                            ...
                            // SEMANTIC FIX: Use improved occurrence detection that catches
                            // more usage patterns (comparisons, patterns, etc.)
                            let is_used_in_crate = {
                                let occurrences = count_occurrences(&name, &crate_wide_content);
                                occurrences > 1
                            };
                    ...
                    Item::Static(s) => {
                        if features.delete_private_const {
                            ...
                            // SEMANTIC FIX: Use improved occurrence detection
                            let is_used_in_crate = {
                                let occurrences = count_occurrences(&name, &crate_wide_content);
                                occurrences > 1
                            };
                    ...
                    Item::Fn(f) => {
                        if features.delete_private_fn {
                            ...
                            // SEMANTIC FIX: Use improved occurrence detection that catches
                            // function pointers, callbacks, and other non-call usage patterns
