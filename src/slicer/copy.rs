...
pub fn copy_source(source_dir: &Path, output_dir: &Path) -> Result<(), String> {
    ...
    // Fix deprecated cargo-clippy cfg attributes
    fix_cargo_clippy_cfg(output_dir)?;
...
/// Fix deprecated `feature = "cargo-clippy"` cfg attributes.
/// Old crates used this pattern, but modern clippy detection is built-in.
/// Replace with plain `clippy` cfg which works with modern Rust.
fn fix_cargo_clippy_cfg(output_dir: &Path) -> Result<(), String> {
    let src_dir = output_dir.join("src");
    let search_dir = if src_dir.exists() { &src_dir } else { output_dir };
    ...
    fn visit_and_fix(dir: &Path) -> Result<(), String> {
        for entry in fs::read_dir(dir).map_err(|e| format!("{}: {}", dir.display(), e))?.flatten() {
            ...
            if path.is_dir() {
                ...
                // Check if file contains cargo-clippy cfg
                if !content.contains("cargo-clippy") {
                    continue;
                }

                // Replace feature = "cargo-clippy" with clippy
                let new_content = content
                    .replace(r#"feature = "cargo-clippy""#, "clippy")
                    .replace(r#"feature="cargo-clippy""#, "clippy");
