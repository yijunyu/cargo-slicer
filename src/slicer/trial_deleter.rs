...
impl ItemDependencyGraph {
    ...
    pub fn compute_removal_sets(&self, candidates: &HashSet<String>) -> Vec<RemovalSet> {
        ...
        for set in sets {
            let already_covered = unique_sets.iter().any(|existing| {
                set.members.is_subset(&existing.members)
            });
            if !already_covered {
...
fn extract_references_from_item(item: &Item) -> HashSet<String> {
    ...
    // Convert item to token stream and scan for identifiers
    let tokens = quote::quote!(#item).to_string();

    // Find words that look like identifiers
    // Captures: functions (snake_case), constants (SCREAMING_CASE), types (PascalCase)
    for word in tokens.split(|c: char| !c.is_alphanumeric() && c != '_') {
        ...
        {
            // Include all identifiers that start with a letter (not just lowercase)
            // This catches:
            // - snake_case functions: set_times, log_impl
            // - SCREAMING_CASE constants: LOG_LEVEL_NAMES, HEX_CHARS_LOWER
            // - PascalCase types: BytesToHexChars, MaybeStaticStr
...
fn is_rust_keyword(word: &str) -> bool {
    matches!(word,
        ...
        "await" | "unsafe" | "extern" | "true" | "false" | "Some" | "None" |
        "Ok" | "Err" | "Self" | "str" | "bool" | "char" | "usize" | "isize" |
...
fn build_item_dependency_graph(
    ...
    let item_names: HashSet<String> = private_items.values()
        .map(|i| i.name.clone())
        .collect();
    ...
    // Scan source files and extract references
    ...
    for file_path in rs_files {
        ...
        for item in &syntax_tree.items {
            ...
            if let Item::Macro(macro_item) = item {
                let macro_name = macro_item.ident.as_ref()
                    .map(|i| i.to_string())
                    .unwrap_or_else(|| "unknown_macro".to_string());

                let refs = extract_references_from_item(item);
                for ref_name in refs {
                    if item_names.contains(&ref_name) {
                        // Macro body references private item - add dependency to all matches
                        for (to_key, stored) in private_items {
                            if stored.name == ref_name {
            ...
            if let Item::Impl(impl_block) = item {
                ...
                for ref_name in refs {
                    if item_names.contains(&ref_name) {
                        // Add dependencies to ALL items with matching name
                        // (we can't resolve qualified paths, so be conservative)
                        for (to_key, stored) in private_items {
                            if stored.name == ref_name {
                                // Impl block depends on private item
                                graph.add_reference(&format!("__impl__::{}", type_name), to_key);
                                // Don't break - there may be multiple items with same name
                            }
                        }
                    }
                }
            ...
            if let Some((name, _)) = get_item_info(item) {
                let from_key = item_key(&file_path, &name);
                ...
                for ref_name in refs {
                    if item_names.contains(&ref_name) && ref_name != name {
                        // Add dependencies to ALL items with matching name
                        // Since we can't resolve qualified paths like super::module::item,
                        // we conservatively assume any same-named item could be the target
                        for (to_key, stored) in private_items {
                            if stored.name == ref_name {
...
fn run_workspace_check_json(sliced_workspace_dir: &Path) -> JsonCheckResult {
    // Use --all-features to check feature-gated code (e.g., log's serde support)
    let output = Command::new("cargo")
        .args(["check", "--workspace", "--all-features", "--message-format=json"])
        .current_dir(sliced_workspace_dir)
        .output();
...
fn parse_trait_bound(text: &str) -> Option<TraitRequirement> {
    ...
    if parts.len() >= 2 {
        ...
        Some(TraitRequirement { type_name, trait_name })
    } else {
        None
    }
}
...
fn collect_rs_files(crate_dir: &Path, files: &mut Vec<PathBuf>) {
    ...
    let search_dir = if src_dir.exists() { src_dir } else { crate_dir.to_path_buf() };
...
fn delete_items_from_files(crate_dir: &Path, items_to_delete: &HashSet<String>) -> usize {
    ...
    for file_path in rs_files {
        ...
        if filtered_items.len() < original_count {
            ...
            let modified_content = prettyplease::unparse(&filtered_file);
...
fn restore_items(stored_items: &HashMap<String, StoredItem>, items_to_restore: &HashSet<String>) -> usize {
    // Group items by file
    ...
    for name in items_to_restore {
        if let Some(stored) = stored_items.get(name) {
            items_by_file.entry(stored.file_path.clone())
                .or_default()
                .push(stored);
        }
    }
    ...
    for (file_path, items) in items_by_file {
        ...
        let modified_content = prettyplease::unparse(&restored_file);
        let _ = fs::write(&file_path, modified_content);
    }
...
pub fn run_trial_deletion(
    ...
    let filtered_crate_dirs: Vec<&PathBuf> = crate_dirs.iter()
    ...
    println!("  📋 Found {} private items as deletion candidates", total_candidates);
    ...
    println!("  🗑️  Deleted {} items, running verification...", total_deleted);
    ...
    loop {
        ...
        for symbol in &missing_symbols {
            ...
            let matching_keys: Vec<String> = currently_deleted.iter()
        ...
        if to_restore.is_empty() {
            ...
            if !unmatched_symbols.is_empty() {
                println!("  ⚠️  {} missing symbols not in deleted set: {:?}",
        ...
        let restore_names: Vec<&str> = to_restore.iter()
            .take(3)
            .map(|k| name_from_key(k))
            .collect();

        println!("  🔄 Iteration {}: restoring {} items ({:?}...)",
            iterations,
            to_restore.len(),
            restore_names);
    ...
    println!("  📊 Final: {} deleted, {} restored, {} must-keep",
        final_deletable.len(), total_restored, must_keep.len());
...
pub fn run_trial_deletion_with_sets(
    ...
    for crate_dir in &filtered_crate_dirs {
        ...
        let crate_items: HashMap<String, StoredItem> = all_stored_items.iter()
        ...
        if !crate_items.is_empty() {
            ...
            for (key, deps) in graph.references {
                combined_graph.references.entry(key).or_default().extend(deps);
            }
            for (key, deps) in graph.dependents {
                combined_graph.dependents.entry(key).or_default().extend(deps);
            }
    ...
    println!("  📦 Computed {} removal sets ({} with multiple members)",
        total_sets, multi_member_sets);

    // Preview some interesting sets
    for set in removal_sets.iter().filter(|s| s.size > 1).take(3) {
        let member_names: Vec<&str> = set.members.iter()
            .take(4)
            .map(|k| name_from_key(k))
            .collect();
        println!("    → Set rooted at '{}': {} members {:?}{}",
    ...
    let sets_to_test: Vec<_> = removal_sets.into_iter()
    ...
    for set in sets_to_test {
        ...
        if success {
            ...
            if sets_tested <= 10 || sets_tested % 10 == 0 {
                println!("    ✅ Set '{}' ({} items) deletable",
                    name_from_key(&set.root), set.size);
            }
        } else {
            ...
            if sets_tested <= 10 || sets_tested % 10 == 0 {
                println!("    ❌ Set '{}' ({} items) must keep",
                    name_from_key(&set.root), set.size);
            }
        }
    }

    println!("  📊 Final: {} sets tested, {} deletable ({} items), {} must-keep ({} items)",
        sets_tested, sets_deletable, final_deletable.len(),
        sets_tested - sets_deletable, must_keep.len());
...
pub fn run_graph_guided_deletion(
    ...
    let candidate_keys: HashSet<String> = all_stored_items.keys().cloned().collect();
    let mut likely_unused: HashSet<String> = candidate_keys.iter()
        .filter(|item| {
    ...
    println!("  📊 Graph analysis: {} likely unused, {} have dependents",
        likely_unused.len(), items_with_deps);
    ...
    println!("  🗑️  Deleted {} items, running JSON verification...", total_deleted);
    ...
    for iteration in 1..=max_iterations {
        ...
        for symbol in &check_result.missing_symbols {
            let matching_keys: Vec<String> = likely_unused.iter()
                .filter(|key| name_from_key(key) == symbol && !must_keep.contains(*key))
                .cloned()
                .collect();
            to_restore.extend(matching_keys);
        ...
        if to_restore.is_empty() {
            ...
            if total_errors > 0 {
                println!("  ⚠️  {} errors but no matching items to restore, stopping",
                    total_errors);
                if !check_result.missing_symbols.is_empty() {
                    println!("      Missing: {:?}",
                        check_result.missing_symbols.iter().take(5).collect::<Vec<_>>());
                }
                if !check_result.trait_requirements.is_empty() {
                    println!("      Traits: {:?}",
                        check_result.trait_requirements.iter().take(3).collect::<Vec<_>>());
                }
            }
            break;
        }
...
pub fn build_deletion_table(
    crate_dir: &Path,
    max_trials: usize,
) -> DeletionTable {
    let mut table = DeletionTable {
        crate_name: crate_dir.file_name()
...
pub fn apply_verified_deletions(
    crate_dir: &Path,
    table: &DeletionTable,
) -> Result<usize, String> {
...
error[E0408]: variable `STATE` is not bound in all patterns
    ...
    fn test_extract_backtick_content() {
        assert_eq!(extract_backtick_content("cannot find `Foo` in this scope"),
                   Some("Foo".to_string()));
        assert_eq!(extract_backtick_content("unresolved import `crate::bar::Baz`"),
