...
impl BuildHandler {
    ...
    fn handle_virtual_build(&self, cargo_args: &[String]) -> i32 {
        ...
        let lazy_mode = std::env::var("CARGO_SLICER_LAZY").is_ok();

        if cross_crate {
            return self.handle_cross_crate_build(cargo_args);
        }
        if lazy_mode {
            return self.handle_lazy_build(cargo_args);
        }
    ...
    /// Build with lazy compilation (metadata-first two-pass).
    ///
    /// Pass 1: Library crates emit metadata only (no codegen), allowing downstream
    /// crates to start immediately.
    /// Pass 2: After binary crate analysis, re-compile library crates with selective codegen.
    ///
    /// This is triggered by CARGO_SLICER_LAZY=1.
    fn handle_lazy_build(&self, cargo_args: &[String]) -> i32 {
        let rustc_driver = match self.find_rustc_driver() {
            Ok(driver) => driver,
            Err(e) => {
                eprintln!("Error: {}", e);
        ...
        println!("Lazy compilation enabled (metadata-first two-pass)");
        println!("Using driver: {}", rustc_driver.display());
        println!("Using dispatch: {}", dispatch_binary.display());
        ...
        // Optional: Run pre-analysis first for cross-crate seeds
        let workspace_root = std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."));
        if std::env::var("CARGO_SLICER_CROSS_CRATE").is_ok() {
            println!("\n[Pre-analysis] Running cross-crate analysis...");
            match crate::pre_analyze::run_pre_analysis(&workspace_root) {
                Ok(()) => {}
                Err(e) => {
                    eprintln!("Pre-analysis failed: {}", e);
        ...
        // Clean up any stale .lazy-pending files
        ...
        // Run cargo build with lazy compilation dispatch
        println!("\nBuilding with lazy compilation...");
        let mut build_cmd = Command::new("cargo");
        for arg in &extra_args {
            build_cmd.arg(arg);
        }
        ...
        build_cmd.env("CARGO_SLICER_VIRTUAL", "1");
        build_cmd.env("CARGO_SLICER_CODEGEN_FILTER", "1");
        build_cmd.env("CARGO_SLICER_LAZY", "1");
        if let Ok(cache) = std::env::var("CARGO_SLICER_CACHE_DIR") {
            build_cmd.env("CARGO_SLICER_CACHE_DIR", cache);
        }
        ...
        match build_cmd.status() {
            Ok(s) => {
                let exit_code = s.code().unwrap_or(1);
                if exit_code == 0 {
                    println!("\nLazy compilation build complete.");
                }
                exit_code
            }
            Err(e) => {
    ...
    fn find_rustc_driver(&self) -> Result<std::path::PathBuf, String> {
        ...
        if let Ok(exe) = std::env::current_exe() {
            if let Some(dir) = exe.parent() {
                let driver = dir.join(&driver_name);
                if driver.exists() {
                    return Ok(driver);
                }
            }
            ...
            if let Some(dir) = exe.parent() {
                ...
                if dir.ends_with("release") {
                    if let Some(target_dir) = dir.parent() {
                        ...
                        let release_rustc = target_dir.join("release-rustc").join(&driver_name);
                        if release_rustc.exists() {
                            return Ok(release_rustc);
                        }

                        let debug_driver = target_dir.join("debug").join(&driver_name);
        ...
        let common_paths = [
            format!("./target/release-rustc/{}", driver_name),  // Custom profile for rustc driver
            format!("./target/release/{}", driver_name),
            format!("./target/debug/{}", driver_name),
        ];
        ...
        if let Ok(output) = Command::new(which_cmd)
            .arg(format!("cargo-slicer-rustc{}", std::env::consts::EXE_SUFFIX))
            .output()
        {
    ...
    fn find_dispatch_binary(&self) -> Result<std::path::PathBuf, String> {
        let dispatch_name = format!("cargo_slicer_dispatch{}", std::env::consts::EXE_SUFFIX);
        ...
        if let Ok(exe) = std::env::current_exe() {
            if let Some(dir) = exe.parent() {
                let dispatch = dir.join(&dispatch_name);
                if dispatch.exists() {
                    return Ok(dispatch);
                }
            }
        ...
        let common_paths = [
            format!("./target/release-rustc/{}", dispatch_name),
            format!("./target/release/{}", dispatch_name),
            format!("./target/debug/{}", dispatch_name),
        ];
