...
pub struct WatchConfig {
    ...
    pub features_arg: Option<String>,
    /// Whether to activate all features
    pub all_features: bool,
    /// Whether to deactivate default features
...
fn run_cargo_build(project_root: &Path, config: &WatchConfig) -> Result<(), String> {
    let mut cmd = std::process::Command::new("cargo");
    cmd.arg("build");
    if config.build_release {
        cmd.arg("--release");
    }
    if let Some(ref f) = config.features_arg {
                ...
                if state.pending_changes && state.last_change.elapsed() > config.debounce {
                    match state.process_changes(&config) {
                        ...
                        WatchAction::RebuildOnly => {
                            ...
                            match run_cargo_build(&config.project_root, &config) {
