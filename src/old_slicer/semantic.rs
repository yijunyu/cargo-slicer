...
use super::import_decision_log::{logger, DecisionSource, ImportDecision, UsageContext};
...
use crate::types::{CrateIndex, CrateInfo, ParsedItemKind, SemanticSliceResult, UsedItem};
...
fn strip_cfg_test_blocks(source: &str) -> String {
    ...
    static CFG_TEST_MOD: Lazy<Regex> =
        Lazy::new(|| Regex::new(r"#\s*\[\s*cfg\s*\(\s*test\s*\)\s*\]\s*mod\s+\w+\s*\{").unwrap());
...
fn strip_internal_imports(source: &str) -> String {
    ...
    static STRIP_PATTERNS: Lazy<Regex> = Lazy::new(|| {
        Regex::new(
            r"(?m)^\s*(?:use\s+(?:crate\s*::\s*clippy|clippy)|#\s*\[\s*allow\s*\(\s*clippy).*\n?",
        )
        .unwrap()
    });
...
pub fn is_unstable_std_import(import_source: &str) -> bool {
    ...
    let normalized: String = import_source
        .chars()
...
pub fn module_cfg_matches_platform(
    module_path: &str,
            ...
            if let Some(type_name) = path.rsplit("::").next() {
                ...
                if !EXCLUDED_TYPES.contains(&type_name) {
                    ...
                    if let Some(std_path) = std_types.get(type_name) {
                        ...
                        if !is_unstable_std_import(std_path)
                            && !is_private_std_import(std_path)
                            && !is_prelude_type(std_path)
                        {
...
impl StdImportMatcher {
    fn new() -> Self {
        let rules: Vec<PatternRule> = vec![
            // Hash traits
            PatternRule {
                pattern: "impl Hasher for",
                category: ImportCategory::Hasher,
            },
            PatternRule {
                pattern: ": Hasher",
                category: ImportCategory::Hasher,
            ...
            PatternRule {
                pattern: "impl BuildHasher for",
                category: ImportCategory::BuildHasher,
            },
            PatternRule {
                pattern: ": BuildHasher",
                category: ImportCategory::BuildHasher,
            ...
            PatternRule {
                pattern: "impl Hash for",
                category: ImportCategory::Hash,
            },
            PatternRule {
                pattern: ": Hash",
                category: ImportCategory::Hash,
            ...
            // Cmp traits
            PatternRule {
                pattern: "impl Ord for",
                category: ImportCategory::Ord,
            },
            PatternRule {
                pattern: ": Ord",
                category: ImportCategory::Ord,
            ...
            PatternRule {
                pattern: "impl PartialOrd for",
                category: ImportCategory::PartialOrd,
            },
            PatternRule {
                pattern: ": PartialOrd",
                category: ImportCategory::PartialOrd,
            ...
            PatternRule {
                pattern: "impl PartialEq for",
                category: ImportCategory::PartialEq,
            },
            PatternRule {
                pattern: ": PartialEq",
                category: ImportCategory::PartialEq,
            ...
            PatternRule {
                ...
                category: ImportCategory::CmpOrdering,
            },
            PatternRule {
                pattern: "Ordering::Equal",
                category: ImportCategory::CmpOrdering,
            },
            PatternRule {
                pattern: "Ordering::Greater",
                category: ImportCategory::CmpOrdering,
            },
            PatternRule {
                pattern: ": Ordering",
            ...
            PatternRule {
                pattern: "Ordering::Acquire",
                category: ImportCategory::AtomicOrdering,
            },
            PatternRule {
            ...
            PatternRule {
                pattern: "Ordering::AcqRel",
                category: ImportCategory::AtomicOrdering,
            },
            PatternRule {
                pattern: "Ordering::SeqCst",
                category: ImportCategory::AtomicOrdering,
            },
            // Fmt traits
            PatternRule {
                pattern: "impl Display for",
            ...
            PatternRule {
                pattern: "impl Debug for",
                category: ImportCategory::Debug,
            },
            PatternRule {
                pattern: ": Debug",
                category: ImportCategory::Debug,
            ...
            PatternRule {
                pattern: "impl Write for",
                category: ImportCategory::FmtWrite,
            },
            PatternRule {
                pattern: ": Write",
                category: ImportCategory::FmtWrite,
            ...
            PatternRule {
                pattern: "Formatter<",
                category: ImportCategory::Formatter,
            },
            PatternRule {
                pattern: "fmt::Result",
                category: ImportCategory::Formatter,
            ...
            PatternRule {
                pattern: "fmt::",
                category: ImportCategory::FmtModule,
            },
            // IO traits
            PatternRule {
                pattern: "impl Read for",
                category: ImportCategory::Read,
            },
            PatternRule {
                pattern: ": Read",
                category: ImportCategory::Read,
            ...
            PatternRule {
                pattern: "impl BufRead for",
                category: ImportCategory::BufRead,
            },
            PatternRule {
                pattern: ": BufRead",
                category: ImportCategory::BufRead,
            ...
            PatternRule {
                pattern: "impl Seek for",
                category: ImportCategory::Seek,
            },
            PatternRule {
                pattern: ": Seek",
                category: ImportCategory::Seek,
            ...
            PatternRule {
                pattern: "io::",
                category: ImportCategory::IoModule2,
            },
            // Iter traits
            PatternRule {
                pattern: "impl Iterator for",
                category: ImportCategory::Iterator,
            },
            PatternRule {
                pattern: ": Iterator",
                category: ImportCategory::Iterator,
            ...
            PatternRule {
                pattern: "impl IntoIterator for",
                category: ImportCategory::IntoIterator,
            },
            PatternRule {
            ...
            PatternRule {
                pattern: "impl ExactSizeIterator for",
                category: ImportCategory::ExactSizeIterator,
            },
            PatternRule {
                pattern: "impl DoubleEndedIterator for",
                category: ImportCategory::DoubleEndedIterator,
            },
            PatternRule {
                pattern: "impl FusedIterator for",
                category: ImportCategory::FusedIterator,
            },
            // Ops traits
            PatternRule {
                pattern: "impl Deref for",
                category: ImportCategory::Deref,
            },
            PatternRule {
                pattern: ": Deref",
                category: ImportCategory::Deref,
            ...
            PatternRule {
                pattern: "impl DerefMut for",
                category: ImportCategory::DerefMut,
            },
            PatternRule {
                pattern: ": DerefMut",
                category: ImportCategory::DerefMut,
            ...
            PatternRule {
                pattern: "impl Index<",
                category: ImportCategory::Index,
            },
            PatternRule {
                pattern: "impl IndexMut<",
                category: ImportCategory::IndexMut,
            },
            PatternRule {
                pattern: "impl Drop for",
                category: ImportCategory::Drop,
            },
            PatternRule {
                pattern: "RangeBounds",
                category: ImportCategory::RangeBounds,
            },
            // Convert traits
            PatternRule {
                pattern: "impl From<",
            ...
            PatternRule {
                pattern: "impl TryFrom<",
                category: ImportCategory::TryFrom,
            },
            PatternRule {
                pattern: "impl TryInto<",
                category: ImportCategory::TryInto,
            },
            PatternRule {
                pattern: "impl AsRef<",
                category: ImportCategory::AsRef,
            },
            PatternRule {
                pattern: "impl AsMut<",
                category: ImportCategory::AsMut,
            },
            // Default/Clone
            PatternRule {
                pattern: "impl Default for",
                category: ImportCategory::Default,
            },
            PatternRule {
                pattern: ": Default",
                category: ImportCategory::Default,
            ...
            PatternRule {
                pattern: "impl Clone for",
                category: ImportCategory::Clone,
            },
            PatternRule {
                pattern: ": Clone",
                category: ImportCategory::Clone,
            ...
            PatternRule {
                pattern: "PhantomData<",
                category: ImportCategory::PhantomData,
            },
            PatternRule {
                pattern: "PhantomData,",
                category: ImportCategory::PhantomData,
            ...
            PatternRule {
                pattern: "impl Borrow<",
                category: ImportCategory::Borrow,
            },
            PatternRule {
                pattern: ": Borrow<",
                category: ImportCategory::Borrow,
            ...
            PatternRule {
                pattern: "impl BorrowMut<",
                category: ImportCategory::BorrowMut,
            },
            PatternRule {
                pattern: "impl ToOwned for",
                category: ImportCategory::ToOwned,
            },
            PatternRule {
                pattern: "Cow<",
                category: ImportCategory::Cow,
            },
            // Error
            PatternRule {
                pattern: "impl Error for",
                category: ImportCategory::Error,
            },
            PatternRule {
                pattern: ": Error",
                category: ImportCategory::Error,
            ...
            // Str
            PatternRule {
                pattern: "impl FromStr for",
                category: ImportCategory::FromStr,
            },
            // Smart pointers
            PatternRule {
                pattern: "Arc<",
                category: ImportCategory::Arc,
            },
            PatternRule {
                pattern: "Rc<",
                category: ImportCategory::Rc,
            },
            PatternRule {
                pattern: "Mutex<",
                category: ImportCategory::Mutex,
            },
            PatternRule {
                pattern: "RwLock<",
                category: ImportCategory::RwLock,
            },
            PatternRule {
                pattern: "Cell<",
                category: ImportCategory::Cell,
            },
            PatternRule {
                pattern: "RefCell<",
                category: ImportCategory::RefCell,
            },
            PatternRule {
                pattern: "UnsafeCell<",
                category: ImportCategory::UnsafeCell,
            },
            PatternRule {
                pattern: "parking_lot::",
                category: ImportCategory::ParkingLot,
            },
            // Hint
            PatternRule {
                pattern: "hint::",
                category: ImportCategory::Hint,
            },
            PatternRule {
                pattern: "unreachable_unchecked",
                category: ImportCategory::Hint,
            },
            // Mem
            PatternRule {
                pattern: "mem::",
                category: ImportCategory::Mem,
            },
            PatternRule {
                pattern: "MaybeUninit<",
                category: ImportCategory::Mem,
            ...
            PatternRule {
                pattern: "SliceIndex",
                category: ImportCategory::SliceIndex,
            },
            // Atomics
            PatternRule {
                pattern: "AtomicUsize",
                category: ImportCategory::AtomicUsize,
            },
            PatternRule {
                pattern: "AtomicBool",
                category: ImportCategory::AtomicBool,
            },
            PatternRule {
                pattern: "AtomicU8",
                category: ImportCategory::AtomicU8,
            },
            PatternRule {
                pattern: "AtomicU16",
                category: ImportCategory::AtomicU16,
            },
            PatternRule {
                pattern: "AtomicU32",
                category: ImportCategory::AtomicU32,
            },
            PatternRule {
                pattern: "AtomicU64",
                category: ImportCategory::AtomicU64,
            },
            PatternRule {
                pattern: "AtomicPtr",
                category: ImportCategory::AtomicPtr,
            },
            // NonZero
            PatternRule {
                pattern: "NonZeroUsize",
                category: ImportCategory::NonZeroUsize,
            },
            PatternRule {
                pattern: "NonZeroU8",
                category: ImportCategory::NonZeroU8,
            },
            PatternRule {
                pattern: "NonZeroU16",
                category: ImportCategory::NonZeroU16,
            },
            PatternRule {
                pattern: "NonZeroU32",
                category: ImportCategory::NonZeroU32,
            },
            PatternRule {
                pattern: "NonZeroU64",
                category: ImportCategory::NonZeroU64,
            },
            PatternRule {
                pattern: "NonZeroI32",
                category: ImportCategory::NonZeroI32,
            },
            PatternRule {
                pattern: "NonZeroI64",
                category: ImportCategory::NonZeroI64,
            },
            // Sync types
            PatternRule {
                pattern: "Condvar",
                category: ImportCategory::Condvar,
            },
            PatternRule {
                pattern: "Once ",
                category: ImportCategory::Once,
            },
            PatternRule {
                pattern: "Once,",
                category: ImportCategory::Once,
            ...
            PatternRule {
                pattern: "Barrier",
                category: ImportCategory::Barrier,
            },
            // Panic types
            PatternRule {
                pattern: "UnwindSafe",
                category: ImportCategory::UnwindSafe,
            },
            PatternRule {
                pattern: "RefUnwindSafe",
                category: ImportCategory::RefUnwindSafe,
            },
            // Thread
            PatternRule {
                pattern: "Thread ",
                category: ImportCategory::Thread,
            },
            PatternRule {
                pattern: "Thread,",
                category: ImportCategory::Thread,
            ...
            PatternRule {
                pattern: "ThreadId",
                category: ImportCategory::ThreadId,
            },
            // Time
            PatternRule {
                pattern: "Duration",
                category: ImportCategory::Duration,
            },
            PatternRule {
                pattern: "Instant",
                category: ImportCategory::Instant,
            },
            PatternRule {
                pattern: "SystemTime",
                category: ImportCategory::SystemTime,
            },
            // Collections
            PatternRule {
                pattern: "HashMap<",
                category: ImportCategory::HashMap,
            },
            PatternRule {
                pattern: "HashSet<",
                category: ImportCategory::HashSet,
            },
            PatternRule {
                pattern: "BTreeMap<",
                category: ImportCategory::BTreeMap,
            },
            PatternRule {
                pattern: "BTreeSet<",
                category: ImportCategory::BTreeSet,
            },
            PatternRule {
                pattern: "VecDeque<",
                category: ImportCategory::VecDeque,
            },
            PatternRule {
                pattern: "LinkedList<",
                category: ImportCategory::LinkedList,
            },
            PatternRule {
                pattern: "BinaryHeap<",
                category: ImportCategory::BinaryHeap,
            },
            // Path
            PatternRule {
                pattern: "Path ",
                category: ImportCategory::Path,
            },
            PatternRule {
                pattern: "Path,",
                category: ImportCategory::Path,
            ...
            PatternRule {
                pattern: "PathBuf",
                category: ImportCategory::PathBuf,
            },
            PatternRule {
                pattern: "CachePadded",
                category: ImportCategory::CachePadded,
            },
            // FFI
            PatternRule {
                pattern: "CStr",
                category: ImportCategory::CStr,
            },
            PatternRule {
                pattern: "CString",
                category: ImportCategory::CString,
            },
            PatternRule {
                pattern: "OsStr",
                category: ImportCategory::OsStr,
            },
            PatternRule {
                pattern: "OsString",
                category: ImportCategory::OsString,
            },
            PatternRule {
                pattern: "c_void",
                category: ImportCategory::CVoid,
            },
            PatternRule {
                pattern: "c_char",
                category: ImportCategory::CChar,
            },
            PatternRule {
                pattern: "c_int",
                category: ImportCategory::CInt,
            },
            PatternRule {
                pattern: "c_uint",
                category: ImportCategory::CUint,
            },
            PatternRule {
                pattern: "c_long",
                category: ImportCategory::CLong,
            },
            PatternRule {
                pattern: "c_ulong",
                category: ImportCategory::CUlong,
            },
            // IO module
            PatternRule {
                pattern: "io::Result",
                category: ImportCategory::IoModule,
            },
            PatternRule {
                pattern: "io::Error",
                category: ImportCategory::IoModule,
            ...
            PatternRule {
                pattern: "env::",
                category: ImportCategory::Env,
            },
            // Fs
            PatternRule {
                pattern: "fs::",
                category: ImportCategory::Fs,
            },
            // Use statement (for negative matching)
            PatternRule {
                pattern: "use ",
                category: ImportCategory::UseStatement,
            },
        ];
...
static RUST_PRIMITIVES: &[&str] = &[
    ...
    "impl", "mut", "fn", "let", "const", "static", "type", "trait", "struct", "enum", "union",
    "mod", "pub", "crate", "self", "super", "use", "where", "for", "loop", "while", "if", "else",
    "match", "return", "break", "continue", "move", "ref", "async", "await", "dyn", "unsafe",
    "extern",
];

fn build_crate_module_matcher(
    all_paths: &HashSet<String>,
    current_module: &str,
    crate_name: Option<&str>,
) -> Option<(AhoCorasick, Vec<String>)> {
...
fn load_std_types() -> &'static HashMap<String, String> {
    STD_TYPES_CACHE.get_or_init(|| {
        let cache_path = std::env::var("STD_TYPES_CACHE").unwrap_or_else(|_| {
...
fn load_std_modules() -> &'static HashMap<String, String> {
    STD_MODULES_CACHE.get_or_init(|| {
        let cache_path = std::env::var("STD_MODULES_CACHE").unwrap_or_else(|_| {
            let home = std::env::var("HOME").unwrap_or_else(|_| ".".to_string());
...
pub fn debug_scip_impl_blocks(crate_path: &Path) {
    ...
    let Ok(_index) = scip::types::Index::parse_from_bytes(&scip_data) else {
        return;
    };
}
...
pub fn generate_std_from_scip(scip_path: &Path) -> Option<StdScipResult> {
    ...
    let mut process_symbol = |sym: &str| {
        if let Some(import_path) = scip_symbol_to_import(sym) {
            ...
            if let Some(type_name) = import_path.split("::").last() {
                ...
                let is_simd_intrinsic =
                    type_name.starts_with("__") && import_path.contains("::arch::");
    ...
    let modules_map: HashMap<String, String> = modules
        .into_iter()
        .filter_map(|path| {
            let parts: Vec<&str> = path.split("::").collect();
            if parts.len() >= 2 {
...
fn load_cache(env_var: &str, default_name: &str) -> HashMap<String, String> {
    let cache_path = std::env::var(env_var).unwrap_or_else(|_| {
        let home = std::env::var("HOME").unwrap_or_else(|_| ".".to_string());
...
fn scip_symbol_to_import(symbol: &str) -> Option<String> {
    ...
    let path = path
        .trim_end_matches('#') // Remove type marker
    ...
    static GENERIC_PLACEHOLDERS: &[&str] = &[
        "Trait", "Item", "Iter", "Self", "This", "Key", "Value", "Input", "Output", "Error",
        "Target", "Source", "Token",
    ];
...
fn get_ambiguous_type_path(type_name: &str, path: &str) -> Option<&'static str> {
    match type_name {
        ...
        "Once" => Some(if path.contains("iter::") {
            "std::iter::Once"
        } else {
            "std::sync::Once"
        }),
        // Weak can be std::rc::Weak or std::sync::Weak
        "Weak" => Some(if path.contains("rc::") {
            "std::rc::Weak"
        } else {
            "std::sync::Weak"
        }),
        ...
        "Entry" => Some(if path.contains("btree") {
        ...
        "OccupiedEntry" => Some(if path.contains("btree") {
            "std::collections::btree_map::OccupiedEntry"
        } else {
            "std::collections::hash_map::OccupiedEntry"
        }),
        "VacantEntry" => Some(if path.contains("btree") {
...
pub fn analyze_crate_via_scip(crate_path: &Path) -> ScipAnalysis {
    ...
    let scip_data = match try_load_scip_cache(crate_path) {
        ...
        None => {
            ...
            let scip_path =
                std::env::temp_dir().join(format!("slicer_analysis_{}.scip", std::process::id()));
...
fn fix_nested_module_imports_in_all_code(code: &str, module_paths: &HashSet<String>) -> String {
    ...
    for path in module_paths {
        if let Some(first_module) = path.split("::").nth(1) {
            // Skip "crate", get first module
        ...
        if result.contains(&bare_pattern) {
            ...
            for line in lines {
                    ...
                    && !trimmed.contains("crate::")
                {
                    ...
                    let fixed_line =
                        line.replace(&bare_pattern, &format!("use crate::{}::", module_name));
...
fn add_fallback_deps(deps: &mut Vec<String>, code: &str) {
    ...
    if !has_dep(deps, "serde_core")
        && (code.contains("serde_core::") || code.contains("::serde_core::"))
    {
    ...
    if !has_dep(deps, "scopeguard") && has_scopeguard_usage {
        ...
        if code.contains("extern crate scopeguard") {
            log_dep_decision(
                "scopeguard",
                "Found 'extern crate scopeguard' declaration",
                true,
            );
        } else {
    ...
    if !has_dep(deps, "doc-comment") && has_doc_comment_usage {
        ...
        if code.contains("extern crate doc_comment") {
            log_dep_decision(
                "doc-comment",
                "Found 'extern crate doc_comment' declaration",
                true,
            );
        } else {
...
fn fix_self_imports_in_lib(content: &str, crate_name: &str) -> String {
    ...
    for line in content.lines() {
        ...
        if (trimmed.starts_with("pub use crate::") || trimmed.starts_with("use crate::"))
            && trimmed.ends_with(";")
        {
...
fn filter_preserved_imports_excluding_local(
    ...
    let preserved = filter_preserved_imports_with_modules(
        use_statements,
        code,
        current_module,
        all_module_paths,
...
fn rewrite_import_path(import: &str, all_module_paths: &HashSet<String>) -> String {
    ...
    for i in (0..segments.len()).rev() {
        ...
        let module_exists = all_module_paths
            .iter()
            ...
            if i == segments.len() - 1 {
                ...
                if is_pub {
                    return import.replace(
                        &format!("pub use crate::{}::", full_module_path),
                        "pub use crate::",
                    );
                } else {
                    return import
                        .replace(&format!("use crate::{}::", full_module_path), "use crate::");
                }
            } else {
                ...
                if is_pub {
                    ...
                    return import.replace(
                        &format!("use crate::{}::", full_module_path),
    ...
    for stmt in use_statements {
        ...
        if is_crate_import && !current_module.is_empty() {
            ...
            if let Some(import_module) = extract_crate_import_module(&stmt.source) {
                if import_module == current_module
                    || import_module.starts_with(&format!("{}::", current_module))
                {
                    continue;
                }
            }
...
fn combine_imports(preserved: &str, detected: &str) -> String {
    ...
    for line in preserved.lines() {
        ...
        let all_covered =
            !symbols.is_empty() && symbols.iter().all(|s| covered_symbols.contains(s));
...
fn normalize_import_path(path: &str) -> String {
    ...
    let normalized = path.replace("core::", "std::").replace("alloc::", "std::");
...
fn deduplicate_import_lines(imports: &str) -> String {
    ...
    for line in imports.lines() {
        ...
        let is_use_stmt = (trimmed.starts_with("use ")
        ...
        if is_use_stmt {
            ...
            if trimmed.contains("use crate::") {
                ...
                let imports_prelude_type = symbol_names
                    .iter()
                    .any(|name| PRELUDE_RESULT_TYPES.contains(&name.as_str()));
                if imports_prelude_type {
                    if debug {
                        crate::debug_log!(
                            "[DedupImport] SKIPPED (prelude Result type from crate): {}",
                            trimmed
                        );
                    }
                    pending_attributes.clear();
                ...
                if trimmed == "use crate::error;" || trimmed == "pub use crate::error;" {
                    if debug {
                        crate::debug_log!(
                            "[DedupImport] SKIPPED (error module shadows std::error): {}",
                            trimmed
                        );
                    }
                    pending_attributes.clear();
                ...
                if imports_private_item {
                    crate::debug_log!(
                        "[Priority 3 DEDUP] SKIPPED (known private item): {}",
                        trimmed
                    );
                    if debug {
                        crate::debug_log!(
                            "[DedupImport] SKIPPED (known private item): {}",
                            trimmed
                        );
                    }
                    pending_attributes.clear();
                ...
                let imports_unicode_table_at_root =
                    UNICODE_TABLE_MODULES.iter().any(|module_name| {
                ...
                if imports_unicode_table_at_root {
                    if debug {
                        crate::debug_log!(
                            "[DedupImport] SKIPPED (unicode_tables module at wrong path): {}",
                            trimmed
                        );
                    }
                    pending_attributes.clear();
            ...
            if debug {
                ...
                crate::debug_log!(
                    "[DedupImport]   Already seen names: {:?}",
                    seen_symbol_names
                );
            }
            ...
            let all_symbols_conflict = !symbol_names.is_empty()
                && symbol_names
                    .iter()
                    .all(|name| seen_symbol_names.contains(name));
            ...
            let new_modules: Vec<&String> = module_paths
                .iter()
                .filter(|path| {
            ...
            let new_symbols: Vec<&String> = symbol_paths
                .iter()
                .filter(|path| {
                    let normalized = normalize_import_path(path);
            ...
            let new_symbol_names: Vec<String> = symbol_names
                .iter()
            ...
            if debug && has_duplicate_symbol_names {
                ...
                let duplicate_names: Vec<_> = symbol_names
                    .iter()
                    .filter(|name| seen_symbol_names.contains(*name))
                    .collect();
            ...
            if (has_duplicate_modules || has_duplicate_symbols || has_duplicate_symbol_names)
                && !new_symbols.is_empty()
            {
                ...
                let new_symbols_filtered: Vec<&String> = new_symbols
                    .iter()
                    .filter(|path| {
                ...
                if !new_symbols_filtered.is_empty() {
                    if let Some(reconstructed) = reconstruct_import_with_new_symbols(
                        trimmed,
                        &symbol_paths,
                        &new_symbols_filtered,
                    ) {
                        if debug {
                    ...
                    if debug {
                        crate::debug_log!(
                            "[DedupImport] SKIPPED (all new symbols filtered): {}",
                            trimmed
                        );
                    }
                    pending_attributes.clear();
            ...
            if debug && !symbol_names.is_empty() {
                crate::debug_log!(
                    "[DedupImport]   Adding to seen_symbol_names: {:?}",
                    symbol_names
                );
            }
...
fn extract_imported_paths(use_stmt: &str) -> (Vec<String>, Vec<String>) {
    ...
    let stmt = strip_use_prefix(use_stmt).trim_end_matches(';').trim();
...
fn reconstruct_import_with_new_symbols(
    ...
    if let Some(prefix) = common_prefix {
        ...
        let items: Vec<String> = new_symbols
            .iter()
            .filter_map(|symbol| {
...
fn extract_function_call_module_paths(
    source: &str,
    index: &CrateIndex,
    crate_name: &str,
) -> HashSet<String> {
...
fn detect_full_module_paths(source: &str) -> (HashSet<String>, HashSet<String>) {
    ...
    let re_crate_only =
        regex::Regex::new(r"crate\s*::\s*([a-z_][a-z0-9_]*(?:\s*::\s*[a-z_][a-z0-9_]*)*)").unwrap();
    ...
    let re_bare =
        regex::Regex::new(r"\b([a-z_][a-z0-9_]*)\s*::\s*([A-Za-z_][A-Za-z0-9_]*)").unwrap();
...
pub fn generate_semantic_sliced_crate_with_needed(
    ...
    for scip_impl_id in &scip_analysis.needed_impl_blocks {
        ...
        if let Some(parsed_name) = scip_impl_to_parsed_name(scip_impl_id) {
            ...
            for item in &index.all_items {
                ...
                let item_normalized_module = item
                    .module_path
                ...
                if item.name == parsed_name
                    && (module_path.is_empty()
                        || item_normalized_module == module_path
    ...
    for (type_name, module_path) in impl_types_included.iter() {
        for item in &index.all_items {
            ...
            if item_type == type_name && &item.module_path == module_path {
                ...
                crate::debug_log!(
                    "[IMPL BLOCK FIX] Including additional impl block for {} in module {}",
                    type_name,
                    norm_module
                );
                modules.entry(norm_module).or_default().push(item);
    ...
    for (type_name, module_path) in types_included.iter() {
        for item in &index.all_items {
            ...
            if impl_type == type_name && &item.module_path == module_path {
                ...
                crate::debug_log!(
                    "[IMPL BLOCK FIX #2] Including impl block for struct/enum {} in module {}",
                    type_name,
                    norm_module
                );
                modules.entry(norm_module).or_default().push(item);
    ...
    let mut private_helpers_to_add: Vec<(String, &crate::types::ParsedItem)> = Vec::new();
    {
        ...
        let const_usages =
            crate::old_slicer::slicing::extract_constant_usages(&all_included_sources);
        ...
        for item in &index.all_items {
            ...
            let is_called = func_calls.contains(&item.name)
                || const_usages.contains(&item.name)
                || type_usages.contains(&item.name);
            if !is_called {
                continue;
            }
        ...
        for dep in &item.dependencies {
            if !crate::old_slicer::parsing::is_primitive_type(dep) {
                for dep_item in index.get_all(dep) {
                    let dep_sig =
                        item_signature_with_module(&dep_item.source, &dep_item.module_path);
    ...
    let (additional_module_paths, _additional_items) =
        detect_full_module_paths(&all_included_sources);
    ...
    let func_call_module_paths =
        extract_function_call_module_paths(&all_included_sources, index, &crate_info.name);
    ...
    for (module_path, items) in &modules {
        let mut public_items: Vec<(String, bool)> = items
            .iter()
            .filter(|item| {
                ...
                final_crate_types
                    ...
                    .and_modify(|existing_path: &mut String| {
                        ...
                        if new_depth < existing_depth
                            || (new_depth == existing_depth && mod_path < existing_path)
                        {
    ...
    let symbol_workflow =
        crate::old_slicer::symbol::create_workflow_from_cache(final_crate_types, external_imports)
    ...
    for (module_path, items) in &modules {
        ...
        let has_unstable = items
            .iter()
            .any(|item| uses_unstable_features(&item.source));
        if has_unstable {
        ...
        let locally_defined: HashSet<String> = items
            .iter()
            .filter_map(|item| {
                ...
                let item_mod_normalized =
                    normalize_module_path(&item.module_path, &crate_info.name);
        ...
        for item in &items {
            ...
            if module_path == "ast::print" {
                ...
                if first_lines
                    .iter()
                    .any(|line| line.contains("use ") && line.contains("Visitor"))
                {
        ...
        let preserved_imports = filter_preserved_imports_excluding_local(
            &index.use_statements,
            &module_content,
            module_path,
            Some(&all_module_paths),
        ...
        let mut already_imported =
            extract_symbols_from_preserved_imports(&index.use_statements, module_path);
        ...
        let child_modules: HashSet<String> = all_module_paths
            .iter()
            .filter_map(|p| {
        ...
        let file_path_str = items
            .first()
            .map(|i| i.file.display().to_string())
            .unwrap_or_default();
        let cfg_conditions: Vec<String> = items
            .iter()
        ...
        let all_imports =
            crate::old_slicer::symbol::fill_import_gaps(&old_imports, &symbol_imports);
        ...
        let encoding_macro_imports =
            generate_encoding_macro_imports(&module_content, &all_imports, Some(&crate_info.name));
    ...
    loop {
        ...
        if iteration > MAX_ITERATIONS {
            eprintln!(
                "Warning: reached max iterations ({}) in third pass module discovery",
                MAX_ITERATIONS
            );
            break;
        }
        ...
        let func_module_paths =
            extract_function_call_module_paths(&all_code, index, &crate_info.name);
        ...
        for (_module_path, items) in &all_newly_added {
            let module_content: String = items
                .iter()
    ...
    for (module_path, items) in &all_newly_added {
        ...
        if !module_content.is_empty() {
            ...
            let child_modules: HashSet<String> = all_module_paths
                .iter()
                .filter(|p| {
                    p.starts_with(&format!("{}::", module_path))
                        && !p[module_path.len() + 2..].contains("::")
                })
            ...
            let parent_mod_file = if parent_path.is_empty() {
                ...
                if mod_rs.exists() {
                    ...
                    if parts.len() > 1 {
                        ...
                        let parent_file = output_dir
                            .join("src")
                            .join(&grandparent)
                            .join(format!("{}.rs", parts.last().unwrap()));
                        parent_file
                    } else {
    ...
    for module_path in &all_module_paths {
        if !modules_with_items.contains(module_path) && !module_path.is_empty() {
            ...
            let has_populated_descendants = populated_modules
                .iter()
    ...
    let additional_modules = scan_for_undeclared_modules(
        &output_dir.join("src"),
    ...
    let (lib_content, module_reexports) = generate_lib_rs(
        &modules,
        &final_module_paths,
        &index.use_statements,
        &all_code,
    ...
    let lib_content = if enum_imports.is_empty() {
        ...
        if lib_content.starts_with("//!") {
            ...
            format!(
                "{}\n{}\n{}",
                &lib_content[..first_newline],
                enum_imports.trim(),
                &lib_content[first_newline..].trim_start_matches('\n')
            )
        } else {
    ...
    let mut external_deps = detect_used_dependencies(
        &all_code_fixed,
        crate_info,
        Some(output_dir),
        &final_module_paths,
    ...
    if is_proc_macro && std::env::var("CARGO_SLICER_LOG_DEPENDENCIES").is_ok() {
        crate::debug_log!(
            "[PROC-MACRO] {} is a proc-macro crate - will only include original dependencies",
            crate_info.name
        );
    }
    ...
    for macro_crate in &index.macro_use_crates {
        ...
        if STDLIB_CRATES.contains(&macro_crate.as_str()) {
            if std::env::var("CARGO_SLICER_LOG_DEPENDENCIES").is_ok() {
                crate::debug_log!(
                    "[DepDetect] {} -> SKIPPED: Stdlib crate, not external dependency",
                    macro_crate
                );
            }
            continue;
        ...
        let (cargo_crate_name, dep_line) = match macro_crate.as_str() {
            ...
            _ => {
                ...
                (
                    &*Box::leak(with_dashes.into_boxed_str()),
                    Box::leak(format!("{} = \"*\"", macro_crate.replace('_', "-")).into_boxed_str())
                        as &str,
                )
            }
        };
        ...
        let already_present = external_deps
            .iter()
            .any(|dep| dep.starts_with(&format!("{} =", cargo_crate_name)));
        ...
        if !already_present {
            ...
            if std::env::var("CARGO_SLICER_LOG_DEPENDENCIES").is_ok() {
                crate::debug_log!(
                    "[DepDetect] {} -> ADDED as {}: Found in macro_use_crates",
                    macro_crate,
                    cargo_crate_name
                );
            }
    ...
    let has_std_feature = all_code.contains("feature = \"std\"")
        || all_code.contains("feature = \\\"std\\\"")
        || all_code.contains("feature = 'std'");
    let has_alloc_feature = all_code.contains("feature = \"alloc\"")
        || all_code.contains("feature = \\\"alloc\\\"")
        || all_code.contains("feature = 'alloc'");
...
"#,
    ...
    if let Err(e) =
        crate::cargo_toml::remove_spurious_dependencies(&cargo_toml_path, &final_module_paths)
    {
...
fn write_module_file(
    ...
    let renamed_parts: Vec<String> = module_path
        .split("::")
    ...
    let has_children = all_paths
        .iter()
        .any(|p| p.starts_with(&format!("{}::", module_path)) && p != module_path);
    ...
    let mut child_modules: Vec<&str> = all_paths
        .iter()
        .filter_map(|p| {
    ...
    if crate_name == "libc" && module_path == "unix" {
        ...
        let unwanted_unix_platforms = match target_os {
            ...
            "freebsd" | "openbsd" | "netbsd" | "dragonfly" => vec![
                "linux_like",
                "haiku",
                "hermit",
                "newlib",
    ...
    child_modules.retain(|m| {
        ...
        const FUNCTION_PREFIXES: &[&str] = &[
            ...
            "decode_",
            "encoded_",
            "decoded_",        // Common function prefixes
    ...
    let inline_mod_pattern =
        regex::Regex::new(r"(?:pub\s*(?:\(crate\)\s*)?)?\bmod\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\{")
            .unwrap();
    let inline_modules: HashSet<String> = inline_mod_pattern
    ...
    let type_pattern = regex::Regex::new(
        r"pub(?:\([^)]*\))?\s+(?:struct|enum|type|trait)\s+([a-zA-Z_][a-zA-Z0-9_]*)",
    )
    ...
    let const_pattern = regex::Regex::new(
        r"(?m)^\s*pub(?:\([^)]*\))?\s+(?:const|static)\s+([a-zA-Z_][a-zA-Z0-9_]*)",
    )
...
//                 // crate::debug_log!("[Priority 7 DEBUG] ast::print formatted line: '{}'", trimmed);
    ...
    for module in &stdlib_modules {
        ...
        if final_content.contains(&module_ref) {
            let lines: Vec<String> = final_content
                .lines()
                .map(|line| {
    ...
    final_content = final_content.replace("Result<hir::Hir>", "Result<hir::Hir, Error>");
    ...
    let final_content = {
        ...
        let lines: Vec<&str> = final_content
            .lines()
            .filter(|line| {
                let trimmed = line.trim();
                let is_private_import = PRIVATE_IMPORT_PATTERNS
                    .iter()
                    .any(|pattern| trimmed == *pattern);
                if is_private_import {
                    crate::debug_log!(
                        "[Priority 3 FINAL] Removing private import from {}: {}",
                        module_path,
                        trimmed
                    );
                }
    ...
    if file_path.to_string_lossy().contains("ast/print.rs")
        || file_path.to_string_lossy().contains("ast\\print.rs")
    {
        use std::io::Read;
...
fn generate_enum_variant_imports(content: &str) -> String {
    ...
    static VARIANT_NAME: Lazy<Regex> =
    ...
    for enum_cap in ENUM_DEF.captures_iter(content) {
        ...
        let variants_used: Vec<&String> = variants
            .iter()
            .filter(|var| {
...
fn generate_encoding_macro_imports(
    content: &str,
    existing_imports: &str,
    crate_name: Option<&str>,
) -> String {
...
fn generate_internal_imports(
    ...
    let inline_modules: HashSet<String> = {
        ...
        let inline_mod_pattern =
            regex::Regex::new(r"(?:pub\s*(?:\([^)]*\)\s*)?)?mod\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\{")
                .unwrap();
    ...
    for import in detect_std_imports(content).lines() {
        if !import.is_empty() {
            ...
            if symbols
                .iter()
                .any(|s| inline_modules.contains(s) || locally_defined.contains(s))
            {
                continue;
            }
    ...
    if let Some(path) = file_path {
            ...
            || path.contains("\\x86_64\\")
        {
            ...
            let uses_simd = content.contains("__m128i")
                || content.contains("__m256i")
            ...
            if uses_simd {
                ...
                if content.contains("x86_64")
                    || content.contains("__m128i")
                    || content.contains("__m256i")
                {
    ...
    for idx in found_std_modules {
        ...
        let is_inside_same_named_module =
            current_module == mod_name || current_module.starts_with(&format!("{}::", mod_name));
    ...
    if let Some(crate_name_str) = crate_name {
        if crate_name_str == "encoding_rs" || crate_name_str == "encoding_rs_sliced" {
            ...
            for type_name in &handle_types {
                ...
                let pattern_regex =
                    regex::Regex::new(&format!(r"\b{}\b", regex::escape(type_name))).unwrap();
    ...
    if let Some((crate_automaton, crate_modules)) =
        build_crate_module_matcher(all_paths, current_module, crate_name)
    {
        let found_crate_modules =
            find_crate_module_usages(content, &crate_automaton, &crate_modules);
        for module_path in found_crate_modules {
            ...
            if first_char.is_uppercase() {
                ...
                match index.items.get(type_name) {
                    Some(items) => {
                        ...
                        let has_public_item_at_path = items
                            .iter()
                            .any(|item| item.is_pub && path_matches(&item.module_path));
                        // Also check if there's NO item at this path (might be private and excluded)
                        let item_exists_at_path =
                            items.iter().any(|item| path_matches(&item.module_path));
            ...
            let module_exists = all_paths.contains(first_module)
                || all_paths
                    .iter()
                    .any(|p| p.starts_with(&format!("{}::", first_module)));
            if !module_exists {
                ...
                if !renamed_path.contains("::") {
                    ...
                    if let Some(pm) = populated_modules {
                        let module_exists = pm.contains(first_component)
                            || pm
                                .iter()
                                .any(|p| p.starts_with(&format!("{}::", first_component)));
                        ...
                        let module_exists_in_paths = all_paths.contains(first_component)
                            || all_paths.iter().any(|p| {
                                p == first_component
                                    || p.starts_with(&format!("{}::", first_component))
                            });
                    ...
                    || is_internal_type
                {
                    ...
                    let is_qualified = if word_start_pos >= 2 {
                        ...
                        if &content_bytes[word_start_pos - 2..word_start_pos] == b"::" {
                            ...
                            let prefix =
                                &content_bytes[word_start_pos.saturating_sub(4)..word_start_pos];
                            prefix.ends_with(b":: ")
                                || prefix.ends_with(b" ::")
                                || prefix == b" :: "
                        } else {
                    ...
                    if is_c_ffi_type || is_c_style_type || is_simd_intrinsic {
                        if let Some(import_path) = std_types.get(&current_word) {
                            // Skip imports from unstable APIs (E0658 fix), private items (E0603 fix), and prelude types
                            if !is_unstable_std_import(import_path)
                                && !is_private_std_import(import_path)
                                && !is_prelude_type(import_path)
                            {
                                // Log decision
                                log_import_decision(
                                    &current_word,
                    ...
                    if let Some(items) = index.items.get(&current_word) {
                        ...
                        for item in items {
                            ...
                            let item_module =
                                normalize_module_path(&item.module_path, crate_name.unwrap_or(""));
                            if item_module.is_empty() {
                        ...
                        if !found_in_crate {
                            ...
                            for item in items {
                                ...
                                let item_module = normalize_module_path(
                                    &item.module_path,
                                    ...
                                    let is_better = match &best_item_module {
                                        ...
                                        Some(best) => {
                                            ...
                                            if item_depth != best_depth {
                                                ...
                                                let item_is_primitive =
                                                    item_module.starts_with("primitive");
                                                let best_is_primitive =
                                                    best.starts_with("primitive");
                            ...
                            if let Some(item_module) = best_item_module {
                                ...
                                let import_stmt =
                                    format!("use crate::{}::{};", renamed_module, current_word);
                                    ...
                                    if scip_path.len() <= 1 {
                                        ...
                                        if let Some(pop_mods) = populated_modules {
                                            if let Some(cn) = crate_name {
                                                let scip_module =
                                                    normalize_module_path(&scip_path, cn);
                                                if !pop_mods.contains(&scip_module) {
                                                    // Module isn't actually included, skip this import
                                                } else {
                                                    // Check if we're in the same module (no import needed)
                                                    // Skip if the first path component is an inline module definition (E0255 fix)
                                                    let first_component = scip_path
                                                        .split("::")
                                                        .next()
                                                        .unwrap_or(scip_path);
                                                    if !inline_modules.contains(first_component) {
                                                        let renamed_path =
                                                            rename_reserved_in_path(scip_path);
                                                        let import_stmt = format!(
                                                            "use crate::{}::{};",
                                                            renamed_path, current_word
                                                        );
                                                        imports.insert(import_stmt);
                                                        found_in_crate = true;
                                                    }
                                            ...
                                            // Skip if the first path component is an inline module definition (E0255 fix)
                                            let first_component =
                                                scip_path.split("::").next().unwrap_or(scip_path);
                                            if !inline_modules.contains(first_component) {
                                                let renamed_path =
                                                    rename_reserved_in_path(scip_path);
                                                let import_stmt = format!(
        ...
        if let Some(m) = cap.get(1) {
            ...
            if let Some(items) = index.items.get(func_name) {
                ...
                for item in items {
                    ...
                    let renamed_module = rename_reserved_in_path(&item_module);
                    imports.insert(format!("use crate::{}::{};", renamed_module, func_name));
                    break;
                }
            }
        }
    ...
    for cap in const_pattern.captures_iter(content) {
        if let Some(m) = cap.get(1) {
            ...
            static SKIP_CONSTS: &[&str] = &[
                "OK", "ERR", "SOME", "NONE", "TRUE", "FALSE", "ID", "IO", "OS", "UI", "DB", "API",
                "URL", "URI",
            ];
            ...
            if let Some(items) = index.items.get(const_name) {
                ...
                for item in items {
                    ...
                    let renamed_module = rename_reserved_in_path(&item_module);
                    imports.insert(format!("use crate::{}::{};", renamed_module, const_name));
                    break;
                }
            }
        }
                        ...
                        for item in items {
                            ...
                            if !item_module.is_empty() && all_paths.contains(&item_module) {
                                ...
                                imports.insert(format!(
                                    "use crate::{}::{} as {};",
                                    renamed_module, alias_info.original, alias_info.alias
                                ));
                                break;
                                ...
                                imports.insert(format!(
                                    "use crate::{} as {};",
                                    alias_info.original, alias_info.alias
                                ));
                                break;
                            }
                        }
    ...
    if imports.is_empty() {
        ...
        let filtered: Vec<String> = imports
            .into_iter()
            .filter(|imp| {
        ...
        for cap in pattern.captures_iter(all_code) {
            ...
            if !type_name.is_empty() && !type_to_module.contains_key(&type_name) {
                ...
                for mod_path in all_paths {
                    ...
                    if mod_name == type_lower
                        || mod_name == format!("{}s", type_lower)
                        || mod_name == format!("{}_", type_lower)
                        || type_lower.starts_with(mod_name)
                    {
                    ...
                    if !root_defined_types.contains(&type_name) {
                        ...
                        for mod_path in all_paths {
                            ...
                            if mod_name == type_lower
                                || mod_name == format!("{}s", type_lower)
                                || mod_name == format!("{}_", type_lower)
                                || mod_name.starts_with(&format!("{}_", type_lower))
                            {
                                let fixed_path = mod_path
                                    .split("::")
                                    .map(|p| rename_reserved_module(p))
                    ...
                    if actual_path.starts_with(&prefix) && all_paths.contains(actual_path) {
                        ...
                        if !child_module.is_empty() {
                            ...
                            if type_name == "Visitor" {
                                ...
                                crate::debug_log!(
                                    "  intermediate_path.starts_with('ast'): {}",
                                    intermediate_path.starts_with("ast")
                                );
                                crate::debug_log!(
                                    "  actual_path.starts_with('hir::visitor'): {}",
                                    actual_path.starts_with("hir::visitor")
                                );
                                crate::debug_log!(
                                    "  Would filter: {}",
                        ...
                        if type_name == "Visitor" {
                            ...
                            crate::debug_log!("  actual_path: {}", actual_path);
                            crate::debug_log!(
                                "  all_paths.contains(actual_path): {}",
                                all_paths.contains(actual_path)
                            );
                        }
                    }
...
fn map_type_to_std_module(type_name: &str) -> Option<&'static str> {
    match type_name {
        ...
        "size_t" | "ssize_t" | "off_t" | "off64_t" | "mode_t" | "pid_t" | "uid_t" | "gid_t"
        | "dev_t" | "ino_t" | "nlink_t" | "blksize_t" | "blkcnt_t" | "time_t" | "suseconds_t"
        | "useconds_t" | "clockid_t" | "clock_t" => Some("std::os"),
...
fn generate_lib_rs(
    ...
    let inline_module_names: HashSet<String> = modules
        .get("")
        .map(|items| {
            items
                .iter()
    ...
    let mut top_modules: Vec<&str> = all_paths
        .iter()
        .filter_map(|p| {
            if !p.contains("::") && !p.is_empty() {
                ...
                let prefix = format!("{}::", p);
                let has_populated_children = populated_modules
                    .iter()
    ...
    if crate_name == "libc" {
        ...
        let platform_module = match target_platform {
            "linux" | "android" | "freebsd" | "openbsd" | "netbsd" | "dragonfly" | "macos"
            | "ios" => "unix",
            "windows" => "windows",
            "fuchsia" => "fuchsia",
            "wasi" => "wasi",
        ...
        let platform_modules = vec![
            "unix", "windows", "fuchsia", "wasi", "hermit", "psp", "sgx", "switch", "vxworks",
        ];
            ...
            for type_name in std_types {
                match type_name {
                    ...
                    "size_of" | "align_of" | "offset_of" => {
                        content.push_str(&format!("pub use std::mem::{};\n", type_name))
                    }
                    ...
                    | "Ord" | "Hash" => {} // Handled by Phase 5.2 stdlib_prelude_types
    ...
    if let Some(items) = modules.get("") {
        ...
        let locally_defined: HashSet<String> = sorted_items
            .iter()
            .filter(|item| {
                matches!(
                    item.kind,
        ...
        let root_child_modules: HashSet<String> = all_paths
            .iter()
            .filter(|p| !p.contains("::"))
            .cloned()
            .collect();
        ...
        // Extract context for decision logging
        let file_path_str = sorted_items
            .first()
            .map(|i| i.file.display().to_string())
            .unwrap_or_default();
        let cfg_conditions: Vec<String> = sorted_items
            .iter()
            .filter_map(|i| i.cfg_attr.as_ref())
            .cloned()
            .collect();

        let imports = generate_internal_imports(
            &root_content,
            "",
            index,
            all_paths,
...
fn apply_module_reexports(
    ...
    for (module_path, reexports) in module_reexports {
        ...
        for (type_name, source_module) in reexports {
            ...
            let import_exists = {
                ...
                let grouped_match = content.lines().any(|line| {
                    ...
                    if (line.contains(source_module)
                        || line.contains(&format!(
                            "self::{}",
                            source_module.trim_start_matches("crate::")
                        )))
                        && line.contains('{')
            ...
            if source_module.starts_with("crate::") {
                ...
                reexport_lines.push_str(&format!(
                    "pub use self::{}::{};\n",
                    source_module, type_name
                ));
            }
        ...
        let new_content = format!(
            "// Auto-generated re-exports for crate:: references\n{}\n{}",
            reexport_lines, content
        );

        // Fix E0433 partial module paths after re-exports are added
        let new_content =
            crate::old_slicer::parsing::fix_partial_paths_in_generated_code(&new_content);
...
fn apply_fixes_recursive(
    ...
    use super::source_fix::{
        fix_conflicting_imports, fix_crate_enum_references, fix_crate_imports, fix_duplicate_impls,
        fix_empty_rt_v2_module, fix_external_crate_imports, fix_lib_rs_reexports,
        fix_missing_imports, fix_module_import_conflicts, fix_nested_module_imports,
        fix_private_item_visibility, fix_result_type_alias, fix_spurious_doc_imports,
        fix_trait_struct_confusion, fix_unstable_imports,
    };
    ...
    for entry in fs::read_dir(dir)? {
        ...
        if path.is_file() && path.extension().map(|e| e == "rs").unwrap_or(false) {
                ...
                let lines: Vec<String> = fixed
                    ...
                    .filter_map(|line| {
                        ...
                        if trimmed == "use crate::hir::translate::unicode;" {
                            crate::debug_log!(
                                "[Priority 3 APPLY_FIXES] Replacing private unicode import in {}",
                                path_str
                            );
                        ...
                        if is_private_import {
                            crate::debug_log!(
                                "[Priority 3 APPLY_FIXES] Removing private import from {}: {}",
                                path_str,
                                trimmed
                            );
                            return None;
            ...
            let fixed = if path_str.contains("memchr-sliced") {
                ...
                const MISSING_MODULE_ITEMS: &[&str] = &["vector::", "rarebytes::"];
            ...
            let fixed = if path_str.contains("regex-syntax-sliced") {
                ...
                let lines: Vec<String> = fixed
                    ...
                    .filter_map(|line| {
                        ...
                        if is_phantom {
                            crate::debug_log!(
                                "[Priority 6 APPLY_FIXES] Removing phantom import from {}: {}",
                                path_str,
                                trimmed
                            );
                            return None;
            ...
            if path_str.contains("memchr-sliced") {
                ...
                if is_lib {
                    if fixed.contains("Memchr < ") || fixed.contains("Memchr::") {
                        if !fixed.contains("use crate::iter::Memchr;")
                            && !fixed.contains("use crate::iter::{Memchr")
                        {
                            needs_imports.push("Memchr");
                        }
                    }
                    if fixed.contains("Memchr2 < ") || fixed.contains("Memchr2::") {
                        if !fixed.contains("use crate::iter::Memchr2;")
                            && !fixed.contains("use crate::iter::{")
                        {
                            needs_imports.push("Memchr2");
                        }
                    }
                    if fixed.contains("Memchr3 < ") || fixed.contains("Memchr3::") {
                        if !fixed.contains("use crate::iter::Memchr3;")
                            && !fixed.contains("use crate::iter::{")
                        {
                ...
                if is_c && (fixed.contains(" size_t") || fixed.contains("as size_t")) {
                    ...
                    if fixed_content != fixed {
                        crate::debug_log!(
                            "[Priority 9 APPLY_FIXES] Replacing size_t with usize in {}",
                            path_str
                        );
                ...
                if !needs_imports.is_empty() {
                    ...
                    crate::debug_log!(
                        "[Priority 9 APPLY_FIXES] Adding cross-module imports to {}: {}",
                        path_str,
                        import_line
                    );
            ...
            if path_str.contains("regex-syntax-sliced") && file_name == "parser.rs" {
                ...
                if fixed.contains("use crate::Error;") {
                    crate::debug_log!(
                        "[Priority 10 APPLY_FIXES] Removing wrong Error import from {}",
                        path_str
                    );
                    fixed = fixed.replace("use crate::Error;\n", "");
                }

                if fixed.contains("Result<hir::Hir, Error>")
                    && !fixed.contains("use crate::error::Error;")
                {
                    crate::debug_log!(
                        "[Priority 10 APPLY_FIXES] Adding error::Error import to {}",
                        path_str
                    );
            ...
            if path_str.contains("regex-syntax-sliced") && file_name == "unicode.rs" {
                ...
                if fixed.contains("Err::<IntoIter<Range>, _>(Error::PropertyNotFound)") {
                    crate::debug_log!(
                        "[Priority 10 APPLY_FIXES] Fixing IntoIter<Range> type annotation in {}",
                        path_str
                    );
                    fixed = fixed.replace(
                ...
                if fixed
                    .contains("Err::<std::result::Result<IntoIter<char>, _>, _>(CaseFoldError(()))")
                {
                    crate::debug_log!(
                        "[Priority 10 APPLY_FIXES] Fixing IntoIter<char> type annotation in {}",
                        path_str
                    );
                    fixed = fixed.replace(
            ...
            if path_str.contains("regex-syntax-sliced/src/ast/mod.rs")
                || path_str.contains("regex-syntax-sliced\\src\\ast\\mod.rs")
            {
                ...
                if !mod_rs_path.exists() {
                    ...
                    for child_entry in fs::read_dir(&path)? {
                        ...
                        if child_path.is_dir() && !child_name.starts_with('.') {
                            ...
                            let subdir_has_rs =
                                fs::read_dir(&child_path)?.filter_map(|e| e.ok()).any(|e| {
...
"#;
    ...
    fn test_generate_internal_imports_conflict() {
        ...
        assert!(
            !result.contains("use std::hint;"),
            "Should not import std::hint when local hint module exists. Result: {}",
            result
        );
    }
}
