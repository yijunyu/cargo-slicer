...
pub struct PreAnalysis {
    ...
    pub is_binary: bool,
    /// (path, is_pub, is_function, is_stubbable_hint)
    /// is_stubbable_hint = true if fn AND NOT trait_impl AND NOT generic AND NOT async
    pub defined_items: Vec<(String, bool, bool, bool)>,
    /// (caller, callee)
...
pub fn select_backend(name: &str) -> Box<dyn PreAnalysisBackend + Send + Sync> {
    match name {
...
pub fn run_pre_analysis_with_backend(workspace_root: &Path, backend_name: &str) -> Result<(), String> {
    ...
    // 2. Analyze each crate (in parallel), skipping crates whose source hasn't changed
    let analyses_written = AtomicUsize::new(0);
    let cache_hits = AtomicUsize::new(0);
    let errors: Vec<_> = crates.par_iter()
        .filter_map(|krate| {
            // Check if existing .analysis file is still valid (source hash matches)
            let analysis_filename = format!("{}-{}.analysis",
                krate.package_name.replace('-', "_"), krate.crate_type);
            let analysis_path = cache_dir.join(&analysis_filename);
            if analysis_path.exists() {
                if let Ok(current_hash) = compute_source_hash(&krate.source_path) {
                    if let Ok(existing) = std::fs::read_to_string(&analysis_path) {
                        if let Some(hash_line) = existing.lines()
                            .find(|l| l.starts_with("SOURCE_HASH:"))
                        {
                            if hash_line.trim_start_matches("SOURCE_HASH:") == current_hash {
                                cache_hits.fetch_add(1, Ordering::Relaxed);
                                return None; // Cache hit — skip re-analysis
                            }
                        }
                    }
                }
            ...
            match backend.analyze_crate(krate, &local_crate_names) {
                Ok(analysis) => {
                    if let Err(e) = write_analysis_file(&cache_dir, &analysis) {
                        Some(e)
                    } else {
                        analyses_written.fetch_add(1, Ordering::Relaxed);
                        None
                    }
                }
                Err(e) => {
                    // Primary backend failed — try fast tokenizer (byte-level, no syn)
                    let fast = FastTokenizerBackend;
                    match fast.analyze_crate(krate, &local_crate_names) {
                        Ok(analysis) => {
                            if let Err(e2) = write_analysis_file(&cache_dir, &analysis) {
                                Some(e2)
                            } else {
                                eprintln!("[pre-analyze] {} ({}): syn failed ({}), used fast tokenizer fallback",
                        ...
                        Err(_) => {
                            eprintln!("[pre-analyze] Warning: skipping {} ({}): {}",
                                krate.package_name, krate.crate_type, e);
                            // Last resort: write .skip-driver marker
                            let skip_path = cache_dir.join(
                                format!("{}-{}.skip-driver", krate.rust_name, krate.crate_type));
                            let skip_tmp = skip_path.with_extension("skip-driver.tmp");
                            let _ = std::fs::write(&skip_tmp, "pre-analyze-parse-failure\n");
    ...
    let written = analyses_written.load(Ordering::Relaxed);
    if hits > 0 {
        eprintln!("[pre-analyze] Wrote {} .analysis files, {} cached (unchanged) in {:.1}s",
            written, hits, start.elapsed().as_secs_f64());
    } else {
...
impl PreAnalysisBackend for FastTokenizerBackend {
    ...
    fn analyze_crate(
        ...
        let mut defined_items: Vec<(String, bool, bool, bool)> = Vec::new();
        ...
        for (mod_path, file_path) in &modules {
            ...
            for item in &items {
                ...
                // Detect stubbability from source text for functions:
                // - async fn → unstubbable
                // - fn name<T> (generics) → unstubbable
                // - otherwise → stubbable (may over-estimate: can't detect trait impl context)
                let is_stubbable = is_fn
                    && !is_async_fn(source_bytes, item.start_byte)
                    && !has_fn_generics(source_bytes, item.start_byte, item.name.len())
                    && !is_unsafe_fn(source_bytes, item.start_byte);
                defined_items.push((path.clone(), is_pub, is_fn, is_stubbable));
    ...
    fn analyze_crate(
        ...
        for (mod_path, file_path) in &modules {
            ...
            for tag in &tags {
                ...
                // Ctags backend can't detect generics/async, conservatively mark unstubbable
                defined_items.push((path.clone(), is_pub, is_fn, false));
...
pub fn build_module_tree(crate_root: &Path, crate_name: &str) -> Result<Vec<(String, PathBuf)>, String> {
    let mut result = Vec::new();
    ...
    let parsed = match syn::parse_file(&source) {
        Ok(f) => f,
        Err(_) => {
            // Fallback: byte-level module discovery when syn can't parse (nightly syntax)
            discover_submodules_fast(&source, &root_module, root_dir, &mut result);
            return Ok(result);
        }
    };
        ...
        if let syn::Item::Mod(item_mod) = item {
            ...
            if let Ok(source) = std::fs::read_to_string(&file_path) {
                if let Ok(parsed) = syn::parse_file(&source) {
                    ...
                    // Fallback: byte-level module discovery for nightly syntax
                    discover_submodules_fast(&source, &mod_path, &child_dir, result);
                }
            }
        }
    }
...
/// Recursively discover submodules using byte-level scanning (no syn).
/// Fallback for when syn can't parse files with nightly-only syntax.
fn discover_submodules_fast(
    source: &str,
    parent_module: &str,
    parent_dir: &Path,
    result: &mut Vec<(String, PathBuf)>,
) {
    for mod_name in discover_modules_byte_scan(source) {
        let mod_path = format!("{}::{}", parent_module, mod_name);

        // Resolve file path: try foo.rs first, then foo/mod.rs
        let direct = parent_dir.join(format!("{}.rs", &mod_name));
        ...
        if let Ok(child_source) = std::fs::read_to_string(&file_path) {
            discover_submodules_fast(&child_source, &mod_path, &child_dir, result);
        }
    }
}

/// Check if "async" keyword appears before "fn" at the given position.
/// Handles: `async fn`, `async unsafe fn`, `pub async fn`, etc.
fn is_async_fn(bytes: &[u8], fn_pos: usize) -> bool {
    let mut i = fn_pos;
    // Skip whitespace before "fn"
    while i > 0 && bytes[i - 1].is_ascii_whitespace() { i -= 1; }
    // Skip "unsafe" if present
    if i >= 6 && &bytes[i - 6..i] == b"unsafe" {
        let word_boundary = i < 7 || !(bytes[i - 7].is_ascii_alphanumeric() || bytes[i - 7] == b'_');
        if word_boundary {
            i -= 6;
            while i > 0 && bytes[i - 1].is_ascii_whitespace() { i -= 1; }
        }
    ...
    // Check for "async"
    i >= 5 && &bytes[i - 5..i] == b"async"
        && (i < 6 || !(bytes[i - 6].is_ascii_alphanumeric() || bytes[i - 6] == b'_'))
}

/// Check if a function is declared `unsafe fn`.
/// Looks backwards from the "fn" keyword for an "unsafe" keyword.
/// For `async unsafe fn`, `is_async_fn` catches it first (both are unstubbable).
fn is_unsafe_fn(bytes: &[u8], fn_pos: usize) -> bool {
    let mut i = fn_pos;
    // Skip whitespace before "fn"
    while i > 0 && bytes[i - 1].is_ascii_whitespace() { i -= 1; }
    // Check for "unsafe"
    i >= 6 && &bytes[i - 6..i] == b"unsafe"
        && (i < 7 || !(bytes[i - 7].is_ascii_alphanumeric() || bytes[i - 7] == b'_'))
}

/// Check if a function has generic parameters: `fn name<...>`.
/// `fn_pos` points to the "fn" keyword, `name_len` is the identifier length.
fn has_fn_generics(bytes: &[u8], fn_pos: usize, name_len: usize) -> bool {
    let mut i = fn_pos + 2; // skip "fn"
    let len = bytes.len();
...
/// Byte-level line scanner for `mod name;` declarations.
/// Returns external module names (skips inline `mod name { ... }`).
/// Does not handle `#[path = "..."]` (rare, acceptable for fallback).
fn discover_modules_byte_scan(source: &str) -> Vec<String> {
    ...
    for line in source.lines() {
        ...
        // Strip visibility prefix: pub, pub(crate), pub(super), etc.
        ...
        let after_mod = rest[4..].trim_start();
        let ident_end = after_mod
            .find(|c: char| !c.is_ascii_alphanumeric() && c != '_')
            .unwrap_or(after_mod.len());
        let ident = &after_mod[..ident_end];

        if !ident.is_empty() && after_mod[ident_end..].trim_start().starts_with(';') {
            modules.push(ident.to_string());
        }
    }
...
struct PreAnalysisVisitor<'a> {
    ...
    defined_items: &'a mut Vec<(String, bool, bool, bool)>,
...
impl<'a> Visit<'a> for PreAnalysisVisitor<'_> {
    fn visit_item_fn(&mut self, node: &'a syn::ItemFn) {
        ...
        let is_generic = !node.sig.generics.params.is_empty();
        let is_async = node.sig.asyncness.is_some();
        let is_unsafe = node.sig.unsafety.is_some();
        let is_stubbable = !is_generic && !is_async && !is_unsafe;

        self.defined_items.push((path.clone(), is_pub, true, is_stubbable));
    ...
    fn visit_item_struct(&mut self, node: &'a syn::ItemStruct) {
        ...
        let path = self.item_path(&name);
        self.defined_items.push((path, Self::is_pub(&node.vis), false, false));
    ...
    fn visit_item_static(&mut self, node: &'a syn::ItemStatic) {
        ...
        self.defined_items.push((path.clone(), is_pub, false, false));
        ...
        if let Some(ref trait_ref) = node.trait_ {
            ...
            for item in &node.items {
                if let syn::ImplItem::Fn(method) = item {
                    ...
                    // Trait impl methods are never stubbable
                    self.defined_items.push((method_path.clone(), Self::is_pub(&method.vis), true, false));
            ...
            // Inherent impl
            let impl_is_generic = !node.generics.params.is_empty();
            for item in &node.items {
                if let syn::ImplItem::Fn(method) = item {
                    let method_name = method.sig.ident.to_string();
                    let method_path = format!("{}::{}::{}",
                    ...
                    let is_generic = impl_is_generic || !method.sig.generics.params.is_empty();
                    let is_async = method.sig.asyncness.is_some();
                    let is_unsafe = method.sig.unsafety.is_some();
                    let is_stubbable = !is_generic && !is_async && !is_unsafe;
                    self.defined_items.push((method_path.clone(), Self::is_pub(&method.vis), true, is_stubbable));
    ...
    fn visit_expr_method_call(&mut self, node: &'a syn::ExprMethodCall) {
        // We can't resolve the receiver type, so emit a wildcard edge.
        // The cross-crate BFS will match *::method_name against all
        // defined items with the same last path segment.
        if let Some(ref caller) = self.current_fn {
            let method_name = node.method.to_string();
            self.call_edges.push((caller.clone(), format!("*::{}", method_name)));
        }

        // Continue visiting sub-expressions (receiver, arguments)
...
fn write_analysis_file(cache_dir: &Path, analysis: &PreAnalysis) -> Result<(), String> {
    ...
    for (path, is_pub, is_fn, is_stubbable) in &analysis.defined_items {
        let vis = if *is_pub { "pub" } else { "priv" };
        let kind = if *is_fn { "fn" } else { "type" };
        let stubbable = if *is_stubbable { "stubbable" } else { "" };
        if stubbable.is_empty() {
            content.push_str(&format!("{}\t{}\t{}\n", path, vis, kind));
        } else {
            content.push_str(&format!("{}\t{}\t{}\t{}\n", path, vis, kind, stubbable));
        }
    }
    ...
    // during concurrent access (e.g., pre-analysis running in background).
    let tmp_path = path.with_extension("analysis.tmp");
...
mod tests {
    ...
    fn test_write_and_parse_analysis() {
        ...
        let analysis = PreAnalysis {
            ...
            is_binary: false,
            defined_items: vec![
                ("testcrate::foo".to_string(), true, true, true),
                ("testcrate::bar".to_string(), false, true, true),
            ],
            call_edges: vec![
