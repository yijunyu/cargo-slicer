...
pub fn extract_all_crate_usages(content: &str) -> Vec<(String, UsedItem)> {
    ...
    // Regex patterns for detecting crate references
    // Pattern 1: use crate_name::path
    let use_pattern = Regex::new(r"^\s*use\s+([a-zA-Z_][a-zA-Z0-9_]*)::").unwrap();
    // Pattern 2: qualified paths like crate_name::Type
    let path_pattern = Regex::new(r"\b([a-zA-Z_][a-zA-Z0-9_]*)::([a-zA-Z_][a-zA-Z0-9_:]*)").unwrap();
    ...
    for line in content.lines() {
        ...
        if trimmed.starts_with("use ") {
            if let Some(caps) = use_pattern.captures(trimmed) {
        ...
        for caps in path_pattern.captures_iter(trimmed) {
            let crate_name = &caps[1];
            let path_rest = &caps[2];

            // Skip std/core/alloc and self/super/crate keywords
            if is_sliceable_crate_name(crate_name) {
