...
impl Resolution {
    ...
    pub fn to_import_statement(&self, symbol: &str) -> Option<String> {
        match self {
            ...
            Resolution::Std { path } => Some(format!("use {};", path)),
...
impl SymbolResolver {
    ...
    pub fn resolve(
        ...
        // 2. Check if this is a crate-local type (don't add std import for it)
        ...
        // 3. Check if defined elsewhere in the crate
        if let Some(path) = self.crate_types.get(symbol) {
...
pub fn create_workflow_from_cache(
    ...
    let std_types = crate::semantic::load_std_types_cache();
