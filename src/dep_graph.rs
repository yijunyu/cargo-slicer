...
pub fn analyze_dependency_graph(manifest_path: Option<&Path>) -> Result<DepGraph, String> {
...
pub fn build_multi_version_info(manifest_path: Option<&Path>) -> Result<MultiVersionInfo, String> {
    let mut cmd = Command::new("cargo");
    cmd.args(["metadata", "--format-version", "1"]);
