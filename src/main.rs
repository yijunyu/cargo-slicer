...
use cargo_slicer::semantic;
use cargo_slicer::crate_info;
use cargo_slicer::usage;
use cargo_slicer::slice_all;
use cargo_slicer::dep_graph;
use cargo_slicer::auto_fix;
use cargo_slicer::codegen;
...
fn print_usage() {
    ...
    eprintln!("  --abort-threshold N  Abort if failure rate exceeds N (0.0-1.0, e.g., 0.3 = 30%)");
    eprintln!("  --no-progress      Disable progress bars (for CI/non-interactive)");
    eprintln!("  --measure          Measure LOC reduction (default: true)");
    eprintln!("  --no-measure       Disable LOC measurement");
    eprintln!("  --unicode          Use unicode characters and emojis in output (default)");
    eprintln!("  --ascii            Use only ASCII characters in output");
    eprintln!("  --original-source  Copy original crate sources to crates_to_slice/ (default)");
...
pub fn main() {
    ...
    let mut abort_threshold: Option<f64> = None;  // Abort if failure rate exceeds threshold
    let mut show_progress = true;  // Show progress bars by default
    let mut measure = true;        // Measure LOC by default
    let mut use_unicode = true;    // Use unicode by default
    let mut original_source = true; // Copy original sources by default
    ...
    if analyze_deps || topo_order || slice_all || union_slice {
        ...
        if slice_all {
            ...
            let result = slice_all::slice_all_deps(&graph, &output_base, use_semantic, auto_fix, use_module_slice, use_adaptive, original_source, measure);
            ...
            if measure {
                cargo_slicer::measure::print_measurement_report(&graph, &result.measurements, use_unicode);
            }
        ...
        if union_slice {
            ...
            let result = slice_all::union_slice_deps(&graph, &output_base, auto_fix, parallel, passthrough, profile_crate.as_deref(), abort_threshold, show_progress, original_source, measure);
    ...
    if original_source {
        if let Some(ref info) = crate_info_opt {
            let crates_to_slice_dir = Path::new("crates_to_slice");
            if !crates_to_slice_dir.exists() {
                let _ = fs::create_dir_all(crates_to_slice_dir);
            }
            let dest = crates_to_slice_dir.join(&info.name);
            if !dest.exists() {
                println!("  Copying original source of '{}' to crates_to_slice/...", crate_name);
                if let Err(e) = slice_all::copy_dir_recursive(&info.path, &dest) {
    ...
    if generate_crate {
        if let Some(ref info) = crate_info_opt {
            ...
            if use_adaptive {
                ...
                match result {
                    Ok(result) => {
                        println!("\n=== Semantic Slice Complete{}" , if use_ra_deps { " (rust-analyzer)" } else { "" });
                        println!("  Output: {}/", out_dir.display());
                        println!("  Total parsed: {} items", result.total_parsed);
                        println!("  Items needed: {} (with transitive deps)", result.items_needed);
                        ...
                        if measure {
                            let before_loc = cargo_slicer::measure::measure_loc(&info.path);
                            let after_loc = cargo_slicer::measure::measure_loc(&out_dir);
                            let reduction = if before_loc > 0 {
                                (before_loc as f64 - after_loc as f64) / before_loc as f64 * 100.0
                            } else {
                                0.0
                            };
                            println!("\n=== LOC Reduction ===");
                            println!("  Before: {} LOC", before_loc);
                            println!("  After:  {} LOC", after_loc);
