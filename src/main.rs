...
/// prebuilt_sliced_release_time: optional timing from the initial sliced release build (to avoid rebuilding)
fn run_benchmarks(project_root: &Path, slicing_time_secs: f64, prebuilt_sliced_release_time: Option<f64>) -> Vec<BenchResult> {
    ...
    println!("\n=== Build Benchmarks ===");
    if prebuilt_sliced_release_time.is_some() {
        println!("Running 3 builds + 1 reused: sliced debug, normal release/debug");
        println!("(Reusing timing from initial sliced release build)");
    } else {
        println!("Running 4 builds: sliced release/debug, normal release/debug");
        println!("(Each build starts fresh after cargo clean)");
    }
    println!("(Release builds done first to avoid debug→release cache effects)\n");

    // 1. Sliced release build (reuse prebuilt timing if available)
    if sliced_manifest.exists() {
        ...
        // Reuse prebuilt timing or rebuild from scratch
        let (duration, success) = if let Some(time) = prebuilt_sliced_release_time {
            println!("  Building sliced (release)... {:.2}s ✓ (reusing initial build)", time);
            (time, true)  // Assume success since we already verified it
        } else {
...
/// Run a nightly cargo-script (`-Zscript`) with cargo-slicer enabled.
///
/// Sets `RUSTC_WRAPPER=cargo_slicer_dispatch` (or delegates to `-Zdead-fn-elimination`
/// when the active nightly supports it) and execs `cargo +nightly -Zscript <file> [args...]`.
/// Cache files (`.slicer-cache/`, `.mir-cache/`) are redirected to a tempdir keyed by
/// the script path so they don't litter the user's cwd.
fn run_script(script_args: &[String]) -> ! {
    use std::os::unix::process::CommandExt;
    use std::process::Command;

    let script_path = PathBuf::from(&script_args[0]);
    let forwarded = &script_args[1..];

    // Resolve cargo_slicer_dispatch: prefer alongside the running cargo-slicer binary,
    // fall back to PATH.
    let dispatch = locate_sibling_bin("cargo_slicer_dispatch");

    // Per-script cache dir under $TMPDIR (keyed by absolute path hash).
    let tmp_root = std::env::var_os("TMPDIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("/tmp"));
    let canon = std::fs::canonicalize(&script_path).unwrap_or(script_path.clone());
    let key = path_hash_hex(canon.to_string_lossy().as_bytes());
    ...
    if has_zflag {
        // -Zdead-fn-elimination works without any RUSTC_WRAPPER — registry deps
        // and the script crate go straight through rustc with the in-tree flag.
        cmd.env(
            "CARGO_ENCODED_RUSTFLAGS",
            "-Z\u{1f}dead-fn-elimination",
        );
    ...
    // exec replaces this process; if we return, exec failed.
    let err = cmd.exec();
    eprintln!("cargo-slicer script: failed to exec cargo: {}", err);
    std::process::exit(1);
}
...
fn locate_sibling_bin(name: &str) -> Option<PathBuf> {
    ...
    if let Ok(path) = std::env::var("PATH") {
        for d in path.split(':') {
            let candidate = PathBuf::from(d).join(name);
            if candidate.is_file() {
                return Some(candidate);
            }
        }
...
/// Stable hex hash of a byte slice (FNV-1a 64) — matches the project's `compute_source_hash` style.
fn path_hash_hex(bytes: &[u8]) -> String {
...
fn print_usage() {
    ...
    eprintln!("      --slice-all    Slice both project and dependencies (experimental)");
    eprintln!("      --export-symbols FILE  Export marked symbols to linker script");
    ...
    eprintln!("      --features FEATURES      Space or comma separated list of features to activate");
    eprintln!("      --all-features           Activate all available features");
    eprintln!("      --no-default-features    Do not activate the `default` feature");
    eprintln!();
    eprintln!("Optimization Levels (default: -O3, or -O1 with --use-local-registry):");
    ...
    eprintln!("  -O3                + Graph-guided deletion (default for normal mode)");
    ...
    eprintln!("    --runs N           Number of cold builds per mode (default: 2)");
    eprintln!("    --rollout-fraction F  Fraction of RL time in rollout phase (default: 0.80)");
    eprintln!("    --gpus N           GPU cluster size for cluster-hour equivalents (default: 8)");
    eprintln!("    --project DIR      Project to benchmark (default: current directory)");
    eprintln!("    --db PATH          Path to bench-results.db for persistence");
    eprintln!("  KPIs reported:");
    eprintln!("    1. Cold-build throughput  (samples/hour, baseline vs. cargo-slicer)");
    eprintln!("    2. Incremental feedback latency  (seconds to first cargo check result)");
    eprintln!("    3. Compute cost per valid sample  (accounting for build pass rate)");
    eprintln!("    4. Cluster-hour equivalent  (samples per GPU-hour at given rollout fraction)");
    eprintln!();
    eprintln!("Examples:");
...
pub fn main() {
    ...
    let mut bench_mode = false;  // Default: no benchmarking (faster)
    let mut features = SlicerFeatures::new();  // Feature configuration for aggressive slicing
    let mut optimization_level: Option<u8> = None;  // Will be set based on mode (default: -O3, or -O1 for local registry)
    ...
    let mut slice_all = false;  // Slice both project and dependencies
    let mut use_local_registry = false;  // Use local registry instead of path dependencies for caching
    let mut export_symbols_path: Option<PathBuf> = None;  // Export marked symbols to linker script
    let mut virtual_mode = false;  // Virtual slicing: skip items at codegen without modifying source
    let mut guided_generate_delete = false;  // -g --delete: remove private unreachable fns instead of stubbing

    // Feature selection flags (passed to cargo metadata and build)
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--slice-all" => slice_all = true,
            "--export-symbols" => {
                i += 1;
                if i < args.len() {
                    export_symbols_path = Some(PathBuf::from(&args[i]));
                } else {
                    eprintln!("Error: --export-symbols requires a file path");
                    std::process::exit(1);
                }
            }
            "--virtual" | "-V" => virtual_mode = true,
            "--delete" => guided_generate_delete = true,
            // Cargo feature flags
            "--features" => {
                i += 1;
                if i < args.len() {
                    features_arg = Some(args[i].clone());
                    passthrough_args.push("--features".to_string());
                    passthrough_args.push(args[i].clone());
                } else {
                    eprintln!("Error: --features requires an argument");
                    std::process::exit(1);
                }
            }
            "--all-features" => {
                all_features = true;
                passthrough_args.push("--all-features".to_string());
            }
            "--no-default-features" => {
                no_default_features = true;
                passthrough_args.push("--no-default-features".to_string());
            }
            ...
            "pre-analyze" => {
                // Check for --parser flag in remaining args
                let mut parser_name = std::env::var("CARGO_SLICER_PARSER")
                    .unwrap_or_else(|_| "syn".to_string());
                let mut j = i + 1;
                while j < args.len() {
                    if args[j] == "--parser" && j + 1 < args.len() {
                        parser_name = args[j + 1].clone();
                        j += 2;
                    } else {
                        j += 1;
                    }
                ...
                match cargo_slicer::pre_analyze::run_pre_analysis_with_backend(&workspace_root, &parser_name) {
            ...
            "gcc-bfs" => {
                ...
                // Used by gcc_slicer.sh after the GCC analysis plugin pass.
                // Usage: cargo-slicer gcc-bfs [<cache_dir>]
                let cache_dir = if i + 1 < args.len() {
                ...
                match cargo_slicer::cross_crate_bfs::run_cross_crate_bfs_gcc(&cache_dir) {
                    Ok(()) => std::process::exit(0),
                    Err(e) => {
                        eprintln!("gcc-bfs error: {}", e);
                        std::process::exit(1);
                    }
                }
            }
            "generate" => {
                // Parse remaining args for --output and --delete
                let mut j = i + 1;
                let mut gen_output: Option<PathBuf> = None;
                let mut gen_delete = false;
                while j < args.len() {
                    match args[j].as_str() {
                        "--output" | "-o" if j + 1 < args.len() => {
                            gen_output = Some(PathBuf::from(&args[j + 1]));
                            j += 2;
                        }
                        "--delete" => {
                            gen_delete = true;
                ...
                let project_name = workspace_root
                    ...
                    .and_then(|n| n.to_str())
                    .unwrap_or("project");
                let out_dir = gen_output.unwrap_or_else(|| {
                    PathBuf::from(format!("{}_generated", project_name))
                });
                let stub_mode = if gen_delete {
                    cargo_slicer::generate::StubMode::Delete
                } else {
                    cargo_slicer::generate::StubMode::Replace
                };
                match cargo_slicer::generate::run_generate(&workspace_root, &out_dir, stub_mode) {
                    Ok(()) => std::process::exit(0),
                    Err(e) => {
                        eprintln!("Error: {}", e);
                        std::process::exit(1);
            ...
            "rl-bench" => {
                // RL Training KPI benchmark.
                // Usage: cargo-slicer rl-bench [--runs N] [--rollout-fraction F]
                //                              [--gpus N] [--project DIR] [--db PATH]
                let mut opts = cargo_slicer::rl_bench::RlBenchOptions::default();
                let mut j = i + 1;
                while j < args.len() {
                    match args[j].as_str() {
                        "--runs" if j + 1 < args.len() => {
                            opts.runs = args[j + 1].parse().unwrap_or(2);
                            j += 2;
                        }
                        "--rollout-fraction" if j + 1 < args.len() => {
                            opts.rollout_fraction = args[j + 1].parse().unwrap_or(0.80);
                            j += 2;
                        }
                        "--gpus" if j + 1 < args.len() => {
                            opts.gpus = args[j + 1].parse().unwrap_or(8);
                            j += 2;
                        }
                        "--project" if j + 1 < args.len() => {
                            opts.project_dir = std::path::PathBuf::from(&args[j + 1]);
                            j += 2;
                        }
                        "--db" if j + 1 < args.len() => {
                            opts.db_path = Some(std::path::PathBuf::from(&args[j + 1]));
                            j += 2;
                        }
                        _ => { j += 1; }
                    }
                }
                match cargo_slicer::rl_bench::run_rl_bench(&opts) {
                    Ok(()) => std::process::exit(0),
                    Err(e) => {
                        eprintln!("rl-bench error: {}", e);
                        std::process::exit(1);
                    }
                }
            }
    ...
    // Set default optimization level if not explicitly specified
    if optimization_level.is_none() {
        optimization_level = if use_local_registry {
            Some(1)  // Use -O1 for local registry (safer, avoids over-deletion)
        } else {
            Some(3)  // Use -O3 for normal mode (default)
        };
    }
    ...
    // Override verify for local registry: skip cargo check, use rustc diagnostics instead
    if use_local_registry {
        features.verify = false;
    }
    ...
    if explicit_build_cmd {
        ...
        // Check for --virtual in cargo_args (since it may come after 'build')
        let virtual_in_args = cargo_args.iter().any(|a| a == "--virtual" || a == "-V");
        let filtered_cargo_args: Vec<String> = cargo_args.iter()
            .filter(|a| *a != "--virtual" && *a != "-V")
            .cloned()
            .collect();

        let handler = BuildHandler::new().with_virtual_mode(virtual_mode || virtual_in_args);
        let exit_code = handler.handle_build_command(&filtered_cargo_args);
    ...
    if clean_mode {
        ...
        // Restore Cargo.toml to original version if it was using sliced dependencies
        // This prevents compilation errors when sliced dependencies are cleaned up
        let cargo_toml = project_root.join("Cargo.toml");
        let cargo_toml_orig = project_root.join("Cargo.toml.orig");
        let cargo_toml_sliced = project_root.join("Cargo.toml.sliced");

        // Check if we need to restore (either Cargo.toml.sliced exists OR Cargo.toml has sliced markers)
        let needs_restore = if cargo_toml.exists() {
            if let Ok(content) = fs::read_to_string(&cargo_toml) {
                content.contains("_sliced") || content.contains("Generated by cargo-slicer")
            } else {
                false
            }
        } else {
        ...
        if needs_restore || cargo_toml_sliced.exists() {
            if cargo_toml_orig.exists() {
                println!("  Restoring original Cargo.toml (was using sliced dependencies)...");
                if let Err(e) = fs::copy(&cargo_toml_orig, &cargo_toml) {
                    eprintln!("    Warning: Failed to restore Cargo.toml from Cargo.toml.orig: {}", e);
                } else {
                    println!("    ✓ Restored Cargo.toml from Cargo.toml.orig");
                }
            } else {
                eprintln!("    Warning: Cargo.toml.orig not found");
                eprintln!("    Hint: Run the slicer again to create a backup, or manually restore Cargo.toml");
            }
        }
    ...
    // Handle -g / --generate: vslice-guided source generation
    // Only intercept when `-g` is used without the legacy semantic flags,
    // i.e. when none of -s/-m/-a/-r are set.
    if _generate_crate && !_use_semantic && !_use_module_slice && !_use_adaptive && !_use_ra_deps {
        let project_name = project_root.file_name()
            .and_then(|n| n.to_str())
            .unwrap_or("project");
        ...
        let stub_mode = if guided_generate_delete {
            cargo_slicer::generate::StubMode::Delete
        } else {
            cargo_slicer::generate::StubMode::Replace
        };
        match cargo_slicer::generate::run_generate(&project_root, &out_dir, stub_mode) {
            Ok(()) => return,
            Err(e) => {
                eprintln!("Error: {}", e);
                std::process::exit(1);
    ...
    if watch_mode {
        ...
        let config = WatchConfig {
            ...
            sliceable_crates,
            build_release,
            features_arg: features_arg.clone(),
            all_features,
            no_default_features,
    ...
    if crate_path.is_none() {
        ...
        let result = slice_all::union_slice_deps(
            ...
            all_features,
            no_default_features,
            export_symbols_path.as_deref(),
        );

        let slicing_time_secs = slicing_start.elapsed().as_secs_f64();

        // Report slicing overhead early (before builds) so users can interrupt if desired
        if bench_mode {
            println!("\n=== Slicing Complete ===");
            println!("Slicing overhead: {:.2}s", slicing_time_secs);
            println!("(Benchmarks will start after the initial build. Press Ctrl+C to skip if you only needed the slicing time)\n");
        }

        // If --slice-all flag is set, also analyze the project itself
        if slice_all {
            let mut slicer_config = cargo_slicer::slicer::config::SlicerConfig::default();
            slicer_config.features = features.clone();
            slicer_config.verbose = verbose;

            match cargo_slicer::slicer::project_slicer::slice_project(&project_root, &slicer_config) {
                Ok(_result) => {
                    // Analysis completed successfully
                }
                Err(e) => {
                    eprintln!("\n⚠️  Project analysis failed: {}", e);
                }
            }
        }
        ...
        if sliced_manifest.exists() {
            ...
            // Build the whole workspace by default. If the caller wants to
            // restrict to a specific target, they can pass `--bin NAME` /
            // `--package NAME` / `-p NAME` via passthrough args; we must
            // NOT hard-code `--bin cargo-slicer`, which only happens to match
            // this project and produces bogus "sliced build succeeded" or
            // "error: can't find bin cargo-slicer" results on any other
            // project (e.g., zeroclaw).
            ...
            // Capture build timing for benchmark mode (if bench_mode and release build)
            let is_release_build = passthrough_args.iter().any(|a| a == "--release");
            let build_start = if bench_mode && is_release_build {
            ...
            // Calculate build time if we captured it
            let sliced_release_build_time = build_start.map(|start| start.elapsed().as_secs_f64());
            ...
            match output {
                Ok(output) => {
                    ...
                    if output.status.success() {
                        ...
                        if bench_mode {
                            let results = run_benchmarks(&project_root, slicing_time_secs, sliced_release_build_time);
