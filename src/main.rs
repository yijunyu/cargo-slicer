        ...
        if sliced_manifest.exists() {
            ...
            let display_args = if let Some(ref feat) = extra_features {
                if passthrough_args.is_empty() {
                    format!("build --bin cargo-slicer --features {}", feat)
                } else {
                    format!("build --bin cargo-slicer {} --features {}", passthrough_args.join(" "), feat)
                }
            } else if !passthrough_args.is_empty() {
                format!("build --bin cargo-slicer {}", passthrough_args.join(" "))
            } else {
                "build --bin cargo-slicer".to_string()
            };
            ...
            // Build only the main binary to avoid issues with optional binaries
            // that might require features not enabled in the sliced build
            build_cmd.arg("--bin").arg("cargo-slicer");
