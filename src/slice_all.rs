...
pub fn union_slice_deps(
    ...
    let multi_version_info = match build_multi_version_info(None) {
        Ok(info) => {
    ...
    let phase5_pb = create_progress_bar(&multi, num_to_generate as u64, "Phase 3");
    ...
    if measure {
        ...
        if features.verify {
            ...
            multi.println("  ⏩ Skipping compilation check (fast mode, final build will verify)".to_string()).ok();
        }
